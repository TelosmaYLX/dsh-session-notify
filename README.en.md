...
| Notification media | Two sources for the large image: **per-reason upload** — insert the `{image}` token via "＋ Insert info → Image" in a template and pick a local file (shown as a chip with a thumbnail in the editor, auto-compressed to 512px, saved per reason); **global URL** — an https image URL as fallback (used when the reason has no upload). **Notification icon** (`icon`) takes an https URL; left empty = site default icon. Effective on the system-notification channel only (the in-page toast is a text card); the "Send" test buttons apply them too |
...
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | New **custom notification media**: the template "＋ Insert info" menu gains an **Image** token — insert `{image}` and pick a local file (auto-compressed to 512px, shown as a thumbnail chip in the editor), uploaded per reason and stored in the settings document (stripped from the rendered body, never written into the session log); a global "Notification image URL" and "Notification icon URL" serve as settings-panel fallbacks; the "Send" test buttons apply them too; effective on the system-notification channel only (the in-page toast is a text card) |
