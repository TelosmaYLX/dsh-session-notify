...
| Notification media | Two URL inputs: **notification image** (`image`, shown in the system-notification card below/inside the body) and **notification icon** (`icon`, the small icon on the notification); use https image URLs; left empty = no large image / site default icon respectively; effective on the system-notification channel only (the in-page toast is a text card); the "Send" test buttons apply them too |
...
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | New **custom notification media**: two URL inputs under "Push mode" in the settings panel — "Notification image" and "Notification icon". The image (Notification `image`) is shown in the system-notification card below/inside the body; the icon (Notification `icon`) replaces the small notification icon (left empty = site default); the "Send" test buttons apply them too; effective on the system-notification channel only (the in-page toast is a text card) |
