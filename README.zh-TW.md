...
每輪對話結束時，把「已完成 / 出錯 / 被阻塞 / 達到上限」連同用時、token 消耗寫入會話日誌，並推送瀏覽器系統通知與頁內 toast。內建 5 種語言、4 套風格預設（顏文字可愛 / 艾露貓 / 貓娘 / DeepSeek 娘）、視覺化文案模板編輯器、自訂預設庫，快取命中率與生成速度取自官方投影，與狀態列同口徑。
...
- **預設系統**：內建「預設」基線 + 4 套一鍵風格預設（顏文字可愛 / 艾露貓 / 貓娘 / DeepSeek 娘——標題與 5 原因正文整套風格化文案）；目前設定可另存為自訂預設（`localStorage` 持久化），支援自動編號的未命名預設（`未命名`、`未命名 2`…）、「來自：xxx · 已修改」來源指示、刪除預設。
...
| 版本 | 日期 | 變更 |
| --- | --- | --- |
| **0.1.13** | 2026-08-29 | 新增 **4 套一鍵風格預設**（顏文字可愛 / 艾露貓 / 貓娘 / DeepSeek 娘——標題 + 5 原因正文整套風格化文案，**支援 5 語言**、載入不強制切換介面語言）；清理預設下拉中已廢棄的舊模板名（playful/formal/detail）；README 預設文案範例同步為「會話「{title}」已xx，請點擊查看。」句式（不含用時與消耗） |
