...
每輪對話結束時，把「已完成 / 出錯 / 被阻塞 / 達到上限」連同用時、token 消耗寫入會話日誌，並推送瀏覽器系統通知與頁內 toast。內建 5 種語言、4 套風格預設（顏文字 / 艾露貓 / 貓娘 / DeepSeek 娘）、視覺化文案模板編輯器、自訂預設庫，快取命中率與生成速度取自官方投影，與狀態列同口徑。
...
- **預設系統**：內建「預設」基線 + 4 套一鍵風格預設（顏文字 / 艾露貓 / 貓娘 / DeepSeek 娘——標題與 5 原因正文整套風格化文案）；目前設定可另存為自訂預設（`localStorage` 持久化），支援自動編號的未命名預設（`未命名`、`未命名 2`…）、「來自：xxx · 已修改」來源指示、刪除預設。
...
| 版本 | 日期 | 變更 |
| --- | --- | --- |
| **0.1.16** | 2026-08-30 | **互動修復**：連按 Backspace 不再「先把標籤刪光才輪到文字」——根因是游標停在文字末尾時，刪除邏輯會跳過整段文字誤刪前方的標籤（例：【標籤A】123【標籤B】456【標籤C】末尾按 3 下會變成 123456）；現在只有游標與標籤之間沒有文字時才刪標籤，否則正常刪字元（同樣 3 下得到 【標籤A】123【標籤B】4；Delete 鍵同步修復） |
| **0.1.15** | 2026-08-30 | **互動優化**：「內容」摺疊區**預設展開**（開啟面板即可編輯各原因正文）；刪除標籤後游標**跳過空文字/換行佔位節點**，直接落到真實內容處——不再「連環刪除標籤、刪完標籤才能刪到文字」，可連貫 Back 連續刪除 |
| **0.1.14** | 2026-08-30 | **程式碼審查修復**：刪除預設前彈確認（防誤刪）；刪除目前使用的預設後表單恢復為**宿主已儲存設定**（不再重設為預設，避免誤按儲存覆蓋已套用設定，且語言保留）；儲存期間的新編輯不再被靜默丟棄（提示保留）；儲存/重設改串列寫入（失敗報告已寫入項目數）；媒體「×」同時清除預覽版（無髒 data URI）；按原因圖片參與預設比對（重新整理後正確識別帶圖預設）；用戶端降級標題不再洩漏 `{duration}` 等佔位符；另存為 id 加隨機後綴防衝突；同名預設提示；重設按鈕僅在有修改時可用；除錯日誌超 512KB 自動截斷 |
| **0.1.13** | 2026-08-29 | 新增 **4 套一鍵風格預設**（顏文字 / 艾露貓 / 貓娘 / DeepSeek 娘——標題 + 5 原因正文整套風格化文案，**支援 5 語言**、載入不強制切換介面語言）；清理預設下拉中已廢棄的舊模板名（playful/formal/detail）；README 預設文案範例同步為「會話「{title}」已xx，請點擊查看。」句式（不含用時與消耗） |
