...
| 版本 | 日期 | 變更 |
| --- | --- | --- |
| **0.1.17** | 2026-08-30 | **提問即時提醒（可自訂版）**：AI 呼叫 `ask_user_question` 提問時立即彈窗提醒（獨立投影 `session-complete-notify-question`，宿主渲染標題+內文，回答後自動失效）；提問文案進入設定面板「標題 / 內容」摺疊區（第 6 行「提問」），支援 `{question}` 佔位符（注入 AI 實際提問）與 `{image}` / `{icon}` 媒體開關；4 套風格預設（顏文字 / 艾露貓 / 貓娘 / DeepSeek 娘）補齊 5 語言提問文案；老宿主無提問投影時用戶端自行拼接兜底 |
