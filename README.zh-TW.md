...
| 通知圖片 | 兩個 URL 輸入框：**通知大圖**（`image`，顯示在系統通知卡片正文中/下方）與**通知圖示**（`icon`，通知上的小圖示）；填 https 圖片連結，留空分別 = 不顯示大圖 / 用網站預設圖示；僅系統通知通道生效（頁內 toast 為文字卡片），「傳送」測試按鈕同樣生效 |
...
| 版本 | 日期 | 變更 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | 新增**自訂通知圖片**：設定面板「推送方式」下方新增「通知大圖」與「通知圖示」兩個 URL 輸入框——大圖（Notification `image`）顯示在系統通知卡片正文中/下方，圖示（Notification `icon`）取代通知小圖示（留空用網站預設）；各原因「傳送」測試按鈕同樣生效；僅系統通知通道生效（頁內 toast 為文字卡片） |
