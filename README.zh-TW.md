...
| 通知圖片 | 大圖兩種來源：**按原因上傳**——在模板中透過「＋ 插入資訊 → 圖片」插入 `{image}` 標籤並選擇本機圖片（編輯器內顯示為帶縮圖的標籤，自動壓縮至 512px，隨各原因獨立儲存）；**全域 URL**——填 https 圖片連結兜底（原因未上傳時使用）。**通知圖示**（`icon`）填 https 連結，留空用網站預設圖示。僅系統通知通道生效（頁內 toast 為文字卡片），「傳送」測試按鈕同樣生效 |
...
| 版本 | 日期 | 變更 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | 新增**自訂通知圖片**：模板「＋ 插入資訊」新增**圖片**標籤——插入 `{image}` 並選擇本機圖片（自動壓縮至 512px，編輯器內顯示為帶縮圖的小標籤），按原因獨立上傳、儲存於設定文件（正文渲染時剝除，不寫入會話日誌）；全域「通知大圖 URL」與「通知圖示 URL」作為設定面板輸入框兜底；各原因「傳送」測試按鈕同樣生效；僅系統通知通道生效（頁內 toast 為文字卡片） |
