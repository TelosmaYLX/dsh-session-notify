...
- 文言はツール名と任意の理由のみ（例：「セッションが Bash の承認を待っています。」）。**コマンド引数などの機密内容は含みません**。プッシュ方式とメディア設定も同様に適用されます。
...
[@YiHui-Liu](https://github.com/YiHui-Liu) 氏の [PR #2](https://github.com/TelosmaYLX/dsh-session-notify/pull/2) に感謝します——権限承認の即時通知（`approval/asked` プロジェクション + クライアント側 3 系統フォールバック）。
