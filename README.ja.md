...
| テンプレート × 5 | 折りたたみセクション（**既定で折りたたみ**、パネルをコンパクトに保つ。ヘッダーにカスタマイズ済みの数を表示、クリックで展開）。展開すると各終了理由（完了、エラー、中止、ブロック、出力上限）ごとに独立した Chip エディタ：テキスト + インライン情報カプセル、カーソル位置に挿入、クリックで削除、リアルタイムプレビュー |
