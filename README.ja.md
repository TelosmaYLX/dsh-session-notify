...
| コンテンツ | 折りたたみセクション（**既定で折りたたみ**、クリックで展開）。展開すると各終了理由（完了、エラー、中止、ブロック、出力上限）ごとに**1 行レイアウト**（理由ラベル + Chip エディタ + 「+」挿入ボタン——メニュー展開中は「−」に変化 + **紙飛行機の送信ボタン**。ボタンは角丸矩形で垂直中央揃え）：テキスト + インライン情報カプセル、カーソル位置に挿入；`{image}`/`{icon}` チップは**サムネイルクリックで大図をプレビュー、× クリックでのみ削除**（誤削除防止）、他のチップはクリックで削除 |
