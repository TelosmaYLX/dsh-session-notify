...
| 通知メディア | 大きな画像の 2 つのソース：**理由ごとのアップロード**——テンプレート内で「＋ 情報を挿入 → 画像」から `{image}` トークンを挿入しローカル画像を選択（エディタ内ではサムネイル付きチップとして表示、**512px 幅・通知表示比率 16:9 で中央クロップ**に自動圧縮、理由ごとに保存）。**グローバルの画像/アイコン**——ローカルアップロードに対応（自動圧縮：**画像 512×288（16:9 中央クロップ）、アイコン 128×128（1:1 正方形中央クロップ）**。入力欄は「アップロード済み」のプレースホルダー表示 + 置換/クリアボタン）または https URL をフォールバックとして指定（その理由にアップロードがない場合に使用）。アイコンは空欄ならサイト既定のアイコン。システム通知チャネルのみ有効（ページ内トーストはテキストカード）。「送信」テストボタンも同様に適用 |
| プッシュタイトル | 全理由で共通のタイトルテンプレート（通常の入力欄、ネイティブのプレースホルダー：入力すると消え、空にすると復元）。空欄の場合は各理由でデフォルトタイトル（完了＝タスク完了、エラー＝タスクエラー、中止＝タスク中止、ブロック＝タスクブロック、上限＝タスクが出力上限に到達）。「＋ 情報を挿入」で任意の情報トークンをカーソル位置に挿入（`{title}` セッションタイトル、`{duration}` 所要時間、`{usage}` 消費、`{cache}` キャッシュヒット、`{tps}` 速度） |
...
| `{image}` | カスタム通知イメージのスイッチ：「＋ 情報を挿入」から挿入してローカル画像を選択（512px に自動圧縮）、理由ごとに独立。本文レンダリング時には剥除され、セッションログには書き込まれません | — |
...
| バージョン | 日付 | 変更内容 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | **カスタム通知メディア**を追加：① テンプレートの「＋ 情報を挿入」メニューに**画像**トークンを追加——`{image}` を挿入してローカル画像を選択（**512px 幅・通知表示比率 16:9 で中央クロップ**に自動圧縮、エディタ内ではサムネイル付きチップとして表示）、理由ごとにアップロードして設定ドキュメントに保存（本文レンダリング時に剥除され、セッションログには書き込まれません）。② **通知イメージ/アイコンがローカルアップロードに対応**（自動圧縮：**画像 512×288（16:9 中央クロップ）、アイコン 128×128（1:1 正方形中央クロップ）**。入力欄は「アップロード済み」のプレースホルダー表示 + 置換/クリアボタン、https URL も可）——クロップにより、アップロードした画像が通知カードに完全に表示され、システムの表示領域による切り詰めを防ぎます。③ **プッシュタイトルの「＋ 情報を挿入」**で任意の情報トークンを挿入（セッションタイトル/所要時間/消費/キャッシュヒット/速度）。設定パネルの「通知メディア」エリアに**処理方法の説明**（クロップ比率）と**アップロード後のサムネイルプレビュー**を表示。「送信」テストボタンも同様に適用。システム通知チャネルのみ有効（ページ内トーストはテキストカード） |
