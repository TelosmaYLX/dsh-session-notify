...
| 通知メディア | 2 つの URL 入力欄：**通知イメージ**（`image`、システム通知カードの本文内/下部に表示）と**通知アイコン**（`icon`、通知の小さなアイコン）。https の画像 URL を指定し、空欄ならそれぞれ「大きな画像なし / サイト既定のアイコン」。システム通知チャネルのみ有効（ページ内トーストはテキストカード）。「送信」テストボタンも同様に適用 |
...
| バージョン | 日付 | 変更内容 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | **カスタム通知メディア**を追加：設定パネルの「プッシュ方式」の下に「通知イメージ」「通知アイコン」の 2 つの URL 入力欄を追加。イメージ（Notification `image`）はシステム通知カードの本文内/下部に表示、アイコン（Notification `icon`）は通知の小さなアイコンを置き換え（空欄ならサイト既定）。「送信」テストボタンも同様に適用。システム通知チャネルのみ有効（ページ内トーストはテキストカード） |
