...
| バージョン | 日付 | 変更内容 |
| --- | --- | --- |
| **0.1.17** | 2026-08-30 | **質問の即時通知（カスタマイズ版）**：AI が `ask_user_question` を呼び出すと即座にポップアップ通知（専用プロジェクション `session-complete-notify-question`。ホストがタイトル+本文をレンダリングし、回答後に自動で無効化）；質問の文言は設定パネルの「タイトル / コンテンツ」折りたたみセクション（6 行目の「質問」）に入り、`{question}` プレースホルダー（AI の実際の質問を注入）と `{image}` / `{icon}` メディアスイッチに対応；4 つのスタイルプリセット（顔文字 / アイルー / ネコ娘 / DeepSeekちゃん）に 5 言語の質問文言を追加；古いホストに質問プロジェクションがない場合はクライアントが自前で組み立ててフォールバック |
| **0.1.16** | 2026-08-30 | **操作修正**：Backspace 連打で「タグをすべて削除してからようやく文字に届く」問題を修正——根本原因は、カーソルが文字列の末尾にあるとき、削除判定が文字列を丸ごとスキップして手前のタグを誤削除すること（例：【タグA】123【タグB】456【タグC】の末尾で 3 回押すと 123456 になっていた）。現在はカーソルとタグの間に文字がない場合のみタグを削除し、それ以外は通常の文字削除（同じ 3 回で 【タグA】123【タグB】4 に。Delete キーも同様に修正） |
| **0.1.15** | 2026-08-30 | **操作改善**：「コンテンツ」の折りたたみ領域を**デフォルトで展開**（パネルを開くと各理由の本文をすぐ編集可能）。チップ削除後、カーソルは**空テキスト/改行プレースホルダーを飛ばして**実際の内容に移動——「タグを連続削除してからようやく文字を削除」という状態がなくなり、Back キーでの連続削除が快適に |
| **0.1.14** | 2026-08-30 | **コードレビュー修正**：プリセット削除前に確認ダイアログ（誤削除防止）。使用中のプリセットを削除した際、フォームは**ホストに保存済みの設定**に戻す（デフォルトに戻さない——誤って保存して適用済み設定を上書きする事故を防止、言語も保持）。保存中に行った新しい編集を静かに破棄しない（保留の旨を表示）。保存/リセットを直列書き込みに変更（失敗時は書き込み済み件数を報告）。メディアの「×」でプレビュー版も同時クリア（汚れた data URI を残さない）。理由別画像をプリセット照合に含める（再読み込み後に画像付きプリセットを正しく認識）。クライアントのフォールバックタイトルで `{duration}` などのプレースホルダーが露出しない。別名保存の id にランダム接尾辞を追加（衝突防止）。同名プリセットを警告。リセットボタンは変更がある場合のみ有効。デバッグログは 512KB 超で自動的に切り詰め |
