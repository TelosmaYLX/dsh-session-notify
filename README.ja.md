...
| 通知メディア | 大きな画像の 2 つのソース：**理由ごとのアップロード**——テンプレート内で「＋ 情報を挿入 → 画像」から `{image}` トークンを挿入しローカル画像を選択（エディタ内ではサムネイル付きチップとして表示、512px に自動圧縮、理由ごとに保存）。**グローバル URL**——https の画像 URL をフォールバックとして指定（その理由にアップロードがない場合に使用）。**通知アイコン**（`icon`）は https URL を指定し、空欄ならサイト既定のアイコン。システム通知チャネルのみ有効（ページ内トーストはテキストカード）。「送信」テストボタンも同様に適用 |
...
| バージョン | 日付 | 変更内容 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | **カスタム通知メディア**を追加：テンプレートの「＋ 情報を挿入」メニューに**画像**トークンを追加——`{image}` を挿入してローカル画像を選択（512px に自動圧縮、エディタ内ではサムネイル付きチップとして表示）、理由ごとにアップロードして設定ドキュメントに保存（本文レンダリング時に剥除され、セッションログには書き込まれません）。グローバルの「通知イメージ URL」「通知アイコン URL」を設定パネルのフォールバックとして提供。「送信」テストボタンも同様に適用。システム通知チャネルのみ有効（ページ内トーストはテキストカード） |
