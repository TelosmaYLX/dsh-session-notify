...
| 알림 미디어 | 두 개의 URL 입력란: **알림 이미지**(`image`, 시스템 알림 카드 본문 안/아래에 표시)와 **알림 아이콘**(`icon`, 알림의 작은 아이콘). https 이미지 URL을 입력하고, 비우면 각각 "큰 이미지 없음 / 사이트 기본 아이콘". 시스템 알림 채널에만 적용(페이지 내 토스트는 텍스트 카드). 「보내기」 테스트 버튼에도 동일 적용 |
...
| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | **사용자 지정 알림 미디어** 추가: 설정 패널의 「알림 방식」 아래에 「알림 이미지」와 「알림 아이콘」 두 개의 URL 입력란 추가. 이미지(Notification `image`)는 시스템 알림 카드 본문 안/아래에 표시되고, 아이콘(Notification `icon`)은 알림의 작은 아이콘을 대체(비우면 사이트 기본값). 「보내기」 테스트 버튼에도 동일 적용. 시스템 알림 채널에만 적용(페이지 내 토스트는 텍스트 카드) |
