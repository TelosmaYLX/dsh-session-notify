...
| 템플릿 × 5 | 접이식 섹션(**기본 접힘**, 패널을 컴팩트하게 유지. 헤더에 사용자 지정된 개수 표시, 클릭하면 펼침). 펼치면 각 종료 사유(완료, 오류, 중단, 차단, 출력 상한)마다 별도의 Chip 편집기: 텍스트 + 인라인 정보 캡슐, 커서 위치에 삽입, 클릭으로 제거, 실시간 미리보기 |
