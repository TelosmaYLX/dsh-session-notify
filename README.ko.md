...
대화의 각 턴이 끝날 때 「완료 / 오류 / 차단 / 상한 도달」 상태를 소요 시간, token 소모량과 함께 세션 로그에 기록하고, 브라우저 시스템 알림과 페이지 내 toast를 푸시합니다. **AI가 질문할 때도 즉시 팝업으로 알려드립니다** — 세션 페이지를 지켜볼 필요가 없습니다. 5개 언어, 4가지 스타일 프리셋(카오모지 / 아이루 / 네코무스메 / DeepSeek짱), 시각적 문구 템플릿 편집기, 커스텀 프리셋 라이브러리가 내장되어 있으며, 캐시 적중률과 생성 속도는 공식 프로젝션에서 가져와 상태 표시줄과 동일한 기준을 사용합니다.
...
| 저장 | 호스트 설정 문서에 기록(`language` / `templates` / `titleTemplate` / `titleTemplates` / `pushMode` / `skipSubagents`). 저장 후 「클릭하여 새로고침」 링크 표시 |
...
| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| **0.1.17** | 2026-08-30 | **질문 즉시 알림(커스터마이징 버전)**: AI가 `ask_user_question`을 호출하면 즉시 팝업 알림(전용 프로젝션 `session-complete-notify-question`. 호스트가 제목+본문 렌더링, 답변 후 자동 무효화); 질문 문구는 설정 패널 「제목 / 콘텐츠」 접이식 섹션(6번째 행 「질문」)에 추가되며, `{question}` 플레이스홀더(AI 실제 질문 주입)와 `{image}` / `{icon}` 미디어 스위치 지원; 4가지 스타일 프리셋(카오모지 / 아이루 / 네코무스메 / DeepSeek짱)에 5개 언어 질문 문구 추가; 질문 프로젝션이 없는 구버전 호스트에서는 클라이언트가 직접 조합해 폴백 |
| **0.1.16** | 2026-08-30 | **조작 수정**: Backspace 연타로「태그를 모두 지운 후에야 글자를 지울 수 있는」문제 수정——원인은 커서가 텍스트 끝에 있을 때 삭제 판정이 텍스트 전체를 건너뛰고 앞의 태그를 잘못 삭제한 것(예: 【태그A】123【태그B】456【태그C】 끝에서 3회 누르면 123456이 됨). 이제 커서와 태그 사이에 텍스트가 없을 때만 태그를 삭제하고, 그 외에는 일반 문자 삭제(동일한 3회로 【태그A】123【태그B】4. Delete 키도 동일하게 수정) |
| **0.1.15** | 2026-08-30 | **조작 개선**: 「콘텐츠」 접이식 영역이 **기본으로 펼쳐짐**(패널을 열면 각 사유 본문을 바로 편집 가능). 칩 삭제 후 커서가 **빈 텍스트/줄바꿈 자리 표시자를 건너뛰고** 실제 내용으로 이동——「태그를 모두 지워야 글자를 지울 수 있는」 상태가 사라져 Back 키 연속 삭제가 원활해집니다 |
| **0.1.14** | 2026-08-30 | **코드 리뷰 수정**: 프리셋 삭제 전 확인 다이얼로그(실수 삭제 방지). 사용 중인 프리셋을 삭제하면 폼이 **호스트에 저장된 구성**으로 복원(기본값으로 초기화하지 않음——실수로 저장하여 적용된 구성을 덮어쓰는 사고 방지, 언어도 유지). 저장 중 새 편집 내용을 조용히 버리지 않음(보존 안내 표시). 저장/초기화를 직렬 쓰기로 변경(실패 시 기록된 항목 수 보고). 미디어 「×」로 미리보기 버전도 함께 제거(더티 data URI 없음). 사유별 이미지를 프리셋 매칭에 포함(새로고침 후 이미지 포함 프리셋을 올바르게 인식). 클라이언트 폴백 제목에서 `{duration}` 등 플레이스홀더가 노출되지 않음. 다른 이름으로 저장 id에 랜덤 접미사 추가(충돌 방지). 같은 이름의 프리셋 경고. 초기화 버튼은 변경이 있을 때만 활성화. 디버그 로그는 512KB 초과 시 자동 절단 |
