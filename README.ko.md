...
| 콘텐츠 | 접이식 섹션(**기본 접힘**, 클릭하면 펼침). 펼치면 각 종료 사유(완료, 오류, 중단, 차단, 출력 상한)마다 **한 줄 레이아웃**(사유 라벨 + Chip 편집기 + "+" 삽입 버튼——메뉴 펼침 중에는 "−"로 변경 + **종이비행기 보내기 버튼**. 버튼은 직사각형이며 세로 중앙 정렬): 텍스트 + 인라인 정보 캡슐, 커서 위치에 삽입; `{image}`/`{icon}` 칩은 **썸네일 클릭으로 큰 이미지 미리보기, × 클릭으로만 삭제**(실수 삭제 방지), 다른 칩은 클릭으로 제거 |
