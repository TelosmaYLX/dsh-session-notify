...
| 알림 미디어 | 큰 이미지의 두 가지 소스: **사유별 업로드**——템플릿에서 「＋ 정보 삽입 → 이미지」로 `{image}` 토큰을 삽입하고 로컬 이미지 선택(편집기에서 썸네일이 있는 칩으로 표시, 512px로 자동 압축, 사유별 저장). **전역 이미지/아이콘**——로컬 업로드 지원(자동 압축: 이미지 512px, 아이콘 128px. 입력란에 "업로드됨" placeholder 표시 + 교체/지우기 버튼) 또는 https URL을 폴백으로 지정(해당 사유에 업로드가 없을 때 사용). 아이콘은 비우면 사이트 기본 아이콘. 시스템 알림 채널에만 적용(페이지 내 토스트는 텍스트 카드). 「보내기」 테스트 버튼에도 동일 적용 |
| 알림 제목 | 모든 사유가 공유하는 제목 템플릿(일반 입력 상자, 네이티브 placeholder 안내: 입력하면 사라지고, 비우면 복원). 비워 두면 각 사유별 기본 제목 사용(완료=작업 완료, 오류=작업 오류, 중단=작업 중단, 차단=작업 차단, 상한=작업이 출력 상한 도달). 「＋ 정보 삽입」으로 커서 위치에 원하는 정보 토큰 삽입(`{title}` 세션 제목, `{duration}` 소요 시간, `{usage}` 소모, `{cache}` 캐시 히트, `{tps}` 속도) |
...
| `{image}` | 사용자 지정 알림 이미지 스위치: 「＋ 정보 삽입」에서 삽입하고 로컬 이미지 선택(512px로 자동 압축), 사유별로 독립. 본문 렌더링 시 제거되며 세션 로그에 기록되지 않음 | — |
...
| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | **사용자 지정 알림 미디어** 추가: ① 템플릿의 「＋ 정보 삽입」 메뉴에 **이미지** 토큰 추가——`{image}`를 삽입하고 로컬 이미지 선택(512px로 자동 압축, 편집기에서는 썸네일이 있는 칩으로 표시), 사유별로 업로드하여 설정 문서에 저장(본문 렌더링 시 제거되어 세션 로그에 기록되지 않음). ② **알림 이미지/아이콘 로컬 업로드 지원**(자동 압축: 이미지 512px, 아이콘 128px. 입력란에 "업로드됨" placeholder 표시 + 교체/지우기 버튼, https URL도 가능). ③ **알림 제목의 「＋ 정보 삽입」**으로 원하는 정보 토큰 삽입(세션 제목/소요 시간/소모/캐시 히트/속도). 「보내기」 테스트 버튼에도 동일 적용. 시스템 알림 채널에만 적용(페이지 내 토스트는 텍스트 카드) |
