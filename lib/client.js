      ...
      function ChipEditor(props) {
        ...
        function chipEl(key) {
          ...
          if (key === 'image') {
            ...
            // 图标标签：方形缩略图（1:1）
            var isrc = propsRef.current.iconData
            if (isrc) {
              var ith = document.createElement('img')
              ith.src = isrc
              ith.alt = ''
              ith.style.cssText = 'width:16px;height:16px;object-fit:cover;border-radius:3px;vertical-align:-3px;margin-right:3px'
              el.appendChild(ith)
            }
            el.appendChild(document.createTextNode(label + ' ×'))
          } else {
      ...
      return function SettingsCard() {
        ...
        function save() {
          ...
          for (var i = 0; i < REASON_FIELDS.length; i++) {
            ...
            images[k] = String(flat['tpl-' + k]).indexOf('{image}') >= 0 ? flat['img-' + k] : ''
            icons[k] = String(flat['tpl-' + k]).indexOf('{icon}') >= 0 ? flat['icon-' + k] : ''
          }
          setNote(t.saving)
          var tasks = [
        ...
        /** 为某原因上传媒体（大图 img / 图标 icon）：文件选择 → 压缩（含裁切）→ 写入 img-<field> / icon-<field> 草稿。 */
          ...
          try {
            ...
            input.onchange = function () {
              ...
              compressImageFile(file, function (dataUri) {
                input.remove()
                if (dataUri) {
                  setVal(key + '-' + field, dataUri)
                  log(key + ' picked for ' + field + ' (' + Math.round(dataUri.length / 1024) + 'KB)')
                } else {
                  setNote(t.imageFailed)
        ...
        function sendTestNotice(field) {
          try {
            ...
            var title = String(titleTpl).replace(/\{image\}/g, '').replace(/\{icon\}/g, '').replaceAll('{title}', samples.title).trim() || samples.title
