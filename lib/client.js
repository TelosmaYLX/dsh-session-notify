      ...
      return function SettingsCard() {
        ...
        function renameCustom() {
          ...
          var next = customPresets.map(function (cp) {
            if (cp.id === renameTarget) { renamed = Object.assign({}, cp, { name: name }); return renamed }
            return cp
          })
          setCustomPresets(next)
          saveCustomPresets(next)
          ...
          if (activePreset && activePreset.id === renameTarget && renamed) {
            setActivePreset({ id: renameTarget, name: name, custom: true, src: renamed })
          }
        }
        ...
        if (!ready) {
          ...
          var rows = [
            ...
            h('div', { key: 'title-fold', style: { display: 'flex', flexDirection: 'column', gap: '6px', padding: '12px 0', borderTop: DIVIDER } },
              h('button', {
                ...
                style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', appearance: 'none', background: 'rgba(127,127,127,0.08)', border: 'none', cursor: 'pointer', padding: '8px 10px', borderRadius: '8px', font: 'inherit', textAlign: 'left', width: '100%' },
              },
          ...
          rows.push(h('div', { key: 'tpl-fold-body', style: { display: 'flex', flexDirection: 'column', paddingLeft: '12px', borderLeft: '1px solid var(--dsw-alias-border-l2, rgba(127,127,127,0.25))' } }, tplRows))
          }
