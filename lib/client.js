  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        titleLabel: '推送标题', titleHint: '所有原因共用；留空时按原因用默认标题', titleTplLabel: '按原因定制标题（可选）', titleTplOpen: '展开', titleTplClose: '收起', tplLabel: '正文模板 × 5', tplCustom: '条已自定义', ttPlaceholders: { completed: '任务已完成', error: '任务出错', aborted: '任务已中止', blocked: '任务被阻塞', 'max-tokens': '任务达到输出上限' },
      ...
      'zh-tw': {
        ...
        titleLabel: '推送標題', titleHint: '所有原因共用；留空時按原因用預設標題', titleTplLabel: '按原因自訂標題（選填）', titleTplOpen: '展開', titleTplClose: '收起', tplLabel: '正文模板 × 5', tplCustom: '條已自訂', ttPlaceholders: { completed: '任務已完成', error: '任務出錯', aborted: '任務已中止', blocked: '任務被阻塞', 'max-tokens': '任務達到輸出上限' },
      ...
      en: {
        ...
        titleLabel: 'Push title', titleHint: 'Shared by all reasons; leave empty for per-reason defaults', titleTplLabel: 'Per-reason titles (optional)', titleTplOpen: 'Expand', titleTplClose: 'Collapse', tplLabel: 'Body templates × 5', tplCustom: 'customized', ttPlaceholders: { completed: 'Task completed', error: 'Task failed', aborted: 'Task aborted', blocked: 'Task blocked', 'max-tokens': 'Task hit the output cap' },
      ...
      ja: {
        ...
        titleLabel: 'プッシュタイトル', titleHint: '全理由共通；空欄なら理由ごとのデフォルトを使用', titleTplLabel: '理由ごとのタイトル（任意）', titleTplOpen: '展開', titleTplClose: '折りたたむ', tplLabel: '本文テンプレート × 5', tplCustom: '件カスタマイズ済み', ttPlaceholders: { completed: 'タスク完了', error: 'タスクエラー', aborted: 'タスク中止', blocked: 'タスクがブロックされました', 'max-tokens': '出力上限に到達' },
      ...
      ko: {
        ...
        titleLabel: '푸시 제목', titleHint: '모든 이유 공통；비우면 이유별 기본값 사용', titleTplLabel: '이유별 제목 (선택)', titleTplOpen: '펼치기', titleTplClose: '접기', tplLabel: '본문 템플릿 × 5', tplCustom: '개 사용자 지정됨', ttPlaceholders: { completed: '작업 완료', error: '작업 오류', aborted: '작업 중단됨', blocked: '작업 차단됨', 'max-tokens': '출력 한도 도달' },
    ...
    function makeSettingsCard(react, ctx) {
      ...
      return function SettingsCard() {
        ...
        var tplOpenState = useState(false) // 「正文模板 × 5」折叠区展开状态（默认收起，保持面板简洁）
        ...
        /** 通知媒体卡片：空态 = 圆角矩形 + 号（点击上传）；有图 = 缩略图 + 右上角 ×（点击删除）。
        ...
        function mediaFieldRow(labelKey, key, maxSize, cropAspect) {
          ...
          var w = square ? '64px' : '120px'
          var hh = square ? '64px' : '68px' // 注意：不可命名为 h（会遮蔽模块级 react.createElement）
          var cardBorder = '1px solid var(--dsw-alias-border-l2, rgba(127,127,127,0.35))'
          return fieldRow(t[labelKey], h('div', { style: { position: 'relative', width: w, height: hh } },
            hasMedia
              ? h('img', {
                src: val, alt: '', title: t.clearMedia,
                style: {
                  width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px', display: 'block',
                  border: cardBorder, background: 'rgba(127,127,127,0.12)',
                },
                onError: function (e) { e.target.style.visibility = 'hidden' },
              })
              : h('button', {
                type: 'button',
                title: labelKey === 'imageUrl' ? t.imagePickTitle : t.iconPickTitle,
        ...
        if (!ready) {
          ...
          // 正文模板折叠区（默认收起，保持面板简洁）：头部显示已自定义的条数，展开后才是 5 条原因编辑器
          var tplCustom = 0
          for (var tfi = 0; tfi < REASON_FIELDS.length; tfi++) {
            if (templateOf(cur, REASON_FIELDS[tfi]) !== '') tplCustom++
          }
          rows.push(h('div', {
            key: 'tpl-fold',
            style: { display: 'flex', flexDirection: 'column', gap: '6px', padding: '12px 0', borderTop: DIVIDER },
            ...
            h('button', {
              ...
              h('span', { style: { fontSize: '13px', color: 'var(--dsw-alias-label-secondary, inherit)' } },
                t.tplLabel + (tplCustom > 0 ? ' (' + tplCustom + ' ' + t.tplCustom + ')' : '')),
              h('span', { style: { fontSize: '12px', color: SUBTLE } }, tplOpen ? t.titleTplClose : t.titleTplOpen),
            ),
          ))
