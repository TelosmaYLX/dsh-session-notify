...
window.__ModuleLoader__.load({
  id: '@dsh-external/dsh-session-notify',
  factory: (require) => {
    var module = { exports: {} }
    ...
    var I18N = {
      zh: {
        ...
        tokens: { label: '完成标签', duration: '用时', usage: '消耗', error: '错误', cache: '缓存命中', tps: '速度 TPS' },
        samples: { label: '会话已完成', duration: '12 秒', usage: '1,240 输入 / 3,560 输出', error: '连接超时', cache: '96.5%', tps: '92 tok/s' },
      ...
      'zh-tw': {
        ...
        tokens: { label: '完成標籤', duration: '用時', usage: '消耗', error: '錯誤', cache: '快取命中', tps: '速度 TPS' },
        samples: { label: '會話已完成', duration: '12 秒', usage: '1,240 輸入 / 3,560 輸出', error: '連線逾時', cache: '96.5%', tps: '92 tok/s' },
      ...
      en: {
        ...
        tokens: { label: 'Label', duration: 'Duration', usage: 'Usage', error: 'Error', cache: 'Cache hit', tps: 'Speed TPS' },
        samples: { label: 'Session completed', duration: '12s', usage: '1,240 in / 3,560 out', error: 'connection timeout', cache: '96.5%', tps: '92 tok/s' },
      ...
      ja: {
        ...
        tokens: { label: '完了ラベル', duration: '所要時間', usage: '消費', error: 'エラー', cache: 'キャッシュヒット', tps: '速度 TPS' },
        samples: { label: 'セッション完了', duration: '12 秒', usage: '1,240 入力 / 3,560 出力', error: '接続タイムアウト', cache: '96.5%', tps: '92 tok/s' },
      ...
      ko: {
        ...
        tokens: { label: '완료 라벨', duration: '소요 시간', usage: '소모', error: '오류', cache: '캐시 히트', tps: '속도 TPS' },
        samples: { label: '세션 완료', duration: '12초', usage: '1,240 입력 / 3,560 출력', error: '연결 시간 초과', cache: '96.5%', tps: '92 tok/s' },
      ...
      function pushCompletion(id, s) {
        ...
        function attempt() {
          ...
          if (notice || Date.now() - start > 6000) {
            notifyUser(name + (notice ? '' : ' 已完成'), notice || (cwdLine ? cwdLine + ' · ' : '') + '详情见会话内系统消息')
            return
          }
    ...
    function makeSettingsCard(react, ctx) {
      ...
      var TOKEN_KEYS = ['duration', 'usage', 'error', 'cache', 'tps']

      /** 系统预设：仅保留「默认」（作为基线，其余以自定义预设承载）。 */
      var PRESETS = [
        { id: 'default', labelKey: 'default', language: 'zh', templates: { completed: '', error: '', aborted: '', blocked: '', 'max-tokens': '' } },
      ]
      ...
      function parseTemplate(tpl) {
        var parts = []
        var re = /\{(label|duration|usage|error|cache|tps)\}/g
        var last = 0
        var m
      ...
      function flatOf(cur) {
        var out = { language: LANG_IDS.indexOf(cur.language) >= 0 ? cur.language : 'zh', includeDuration: !!cur.includeDuration, includeUsage: !!cur.includeUsage }
      ...
      function ChipEditor(props) {
        ...
        function rebuild(tpl) {
          ...
          editor.textContent = ''
          // 空模板：直接渲染默认内容（真实可编辑：文字 + 可删胶囊），始终可改、无转换
          var shown = typeof tpl === 'string' && tpl !== '' ? tpl : (typeof propsRef.current.hint === 'string' ? propsRef.current.hint : '')
        ...
        function save() {
          ...
          Promise.all(tasks).then(function () {
            ...
            if (shownCustom && shownEntry) {
              ...
              entry = Object.assign({}, shownEntry, { language: lang, templates: templates })
              ...
              entry = { id: targetId, name: null, auto: true, language: lang, templates: templates }
