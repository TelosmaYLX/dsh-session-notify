  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        tokens: { title: '会话标题', label: '完成标签', duration: '用时', usage: '消耗', error: '错误', cache: '缓存命中', tps: '速度 TPS', image: '图片' },
        samples: { title: '示例会话', label: '会话已完成', duration: '12 秒', usage: '1,240 输入 / 3,560 输出', error: '连接超时', cache: '96.5%', tps: '92 tok/s', image: '[图片]' },
        imagePickTitle: '选择本地图片（自动压缩为通知大图）', imageFailed: '图片读取失败',
        upload: '上传', replaceMedia: '替换', clearMedia: '清除', uploadedHint: '已上传本地图片（可点右侧按钮替换或清除）',
        mediaNote: '本地上传会自动处理：大图按通知显示比例 16:9 居中裁切（512×288）、图标 1:1 方形裁切（128×128），裁切后即通知中显示的效果；手动填 https URL 不压缩不裁切',
        ...
        imageUrl: '通知大图', imageHint: '通知卡片中的大图（https 图片 URL），留空不显示', iconUrl: '通知图标', iconHint: '通知小图标（https 图片 URL），留空用站点默认图标',
      ...
      'zh-tw': {
        ...
        tokens: { title: '會話標題', label: '完成標籤', duration: '用時', usage: '消耗', error: '錯誤', cache: '快取命中', tps: '速度 TPS', image: '圖片' },
        samples: { title: '範例會話', label: '會話已完成', duration: '12 秒', usage: '1,240 輸入 / 3,560 輸出', error: '連線逾時', cache: '96.5%', tps: '92 tok/s', image: '[圖片]' },
        imagePickTitle: '選擇本機圖片（自動壓縮為通知大圖）', imageFailed: '圖片讀取失敗',
        upload: '上傳', replaceMedia: '替換', clearMedia: '清除', uploadedHint: '已上傳本機圖片（可點右側按鈕替換或清除）',
        mediaNote: '本機上傳會自動處理：大圖按通知顯示比例 16:9 居中裁切（512×288）、圖示 1:1 方形裁切（128×128），裁切後即通知中顯示的效果；手動填 https URL 不壓縮不裁切',
        ...
        imageUrl: '通知大圖', imageHint: '通知卡片中的大圖（https 圖片 URL），留空不顯示', iconUrl: '通知圖示', iconHint: '通知小圖示（https 圖片 URL），留空用網站預設圖示',
      ...
      en: {
        ...
        tokens: { title: 'Session title', label: 'Label', duration: 'Duration', usage: 'Usage', error: 'Error', cache: 'Cache hit', tps: 'Speed TPS', image: 'Image' },
        samples: { title: 'Example session', label: 'Session completed', duration: '12s', usage: '1,240 in / 3,560 out', error: 'connection timeout', cache: '96.5%', tps: '92 tok/s', image: '[image]' },
        imagePickTitle: 'Pick a local image (auto-compressed for the notification)', imageFailed: 'Failed to read the image',
        upload: 'Upload', replaceMedia: 'Replace', clearMedia: 'Clear', uploadedHint: 'Local image uploaded (use the buttons to replace or clear)',
        mediaNote: 'Local uploads are processed automatically: large images are center-cropped to the notification ratio 16:9 (512×288) and icons to 1:1 square (128×128) — the cropped result is exactly what shows in the notification; manually entered https URLs are used as-is',
        ...
        imageUrl: 'Notification image', imageHint: 'Large image in the notification card (https image URL); empty = none', iconUrl: 'Notification icon', iconHint: 'Notification icon (https image URL); empty = site default icon',
      ...
      ja: {
        ...
        tokens: { title: 'セッションタイトル', label: '完了ラベル', duration: '所要時間', usage: '消費', error: 'エラー', cache: 'キャッシュヒット', tps: '速度 TPS', image: '画像' },
        samples: { title: 'サンプルセッション', label: 'セッション完了', duration: '12 秒', usage: '1,240 入力 / 3,560 出力', error: '接続タイムアウト', cache: '96.5%', tps: '92 tok/s', image: '[画像]' },
        imagePickTitle: 'ローカル画像を選択（通知用に自動圧縮）', imageFailed: '画像の読み込みに失敗しました',
        upload: 'アップロード', replaceMedia: '置換', clearMedia: 'クリア', uploadedHint: 'ローカル画像をアップロード済み（右のボタンで置換・削除）',
        mediaNote: 'ローカルアップロードは自動処理されます：大きな画像は通知表示比率 16:9（512×288）に中央クロップ、アイコンは 1:1 正方形（128×128）。クロップ後の見た目がそのまま通知に表示されます。手動の https URL はそのまま使用されます',
        ...
        imageUrl: '通知イメージ', imageHint: '通知カード内の大きな画像（https の画像 URL）。空欄なら非表示', iconUrl: '通知アイコン', iconHint: '通知アイコン（https の画像 URL）。空欄ならサイト既定のアイコン',
      ...
      ko: {
        ...
        tokens: { title: '세션 제목', label: '완료 라벨', duration: '소요 시간', usage: '소모', error: '오류', cache: '캐시 히트', tps: '속도 TPS', image: '이미지' },
        samples: { title: '예시 세션', label: '세션 완료', duration: '12초', usage: '1,240 입력 / 3,560 출력', error: '연결 시간 초과', cache: '96.5%', tps: '92 tok/s', image: '[이미지]' },
        imagePickTitle: '로컬 이미지 선택(알림용으로 자동 압축)', imageFailed: '이미지를 읽지 못했습니다',
        upload: '업로드', replaceMedia: '교체', clearMedia: '지우기', uploadedHint: '로컬 이미지 업로드됨(오른쪽 버튼으로 교체·삭제)',
        mediaNote: '로컬 업로드는 자동 처리됩니다: 큰 이미지는 알림 표시 비율 16:9(512×288) 중앙 크롭, 아이콘은 1:1 정사각(128×128). 크롭 후 모습이 그대로 알림에 표시됩니다. 수동 https URL은 그대로 사용됩니다',
        ...
        imageUrl: '알림 이미지', imageHint: '알림 카드의 큰 이미지(https 이미지 URL). 비우면 표시 안 함', iconUrl: '알림 아이콘', iconHint: '알림 아이콘(https 이미지 URL). 비우면 사이트 기본 아이콘',
    ...
    exports.apply = function apply(ctx) {
      ...
      /** 通知媒体：iconUrl = 全局小图标 URL；imageData = 该原因上传的大图（images[kind]，data URI）；
       *  imageUrl = 全局大图 URL 兜底。kind 缺失（投影降级）时只用全局 URL。 */
      function mediaOf(kind) {
        var out = { imageUrl: '', iconUrl: '', imageData: '' }
        try {
          if (notifyScope) {
            var st = notifyScope.getSnapshot()
            if (st && st.value) {
      ...
      try {
        if (mode !== 'toast' && typeof Notification !== 'undefined') {
          if (Notification.permission === 'granted') {
            ...
            // 自定义媒体：icon = 通知小图标（空 = 站点默认），image = 通知卡片中的大图
            // （优先按原因上传的 imageData，其次全局 imageUrl）
            var opts = { body: body, tag: PLUGIN_TAG + ':' + Date.now(), silent: false }
            if (media && media.iconUrl) opts.icon = media.iconUrl
    ...
    function makeSettingsCard(react, ctx) {
      ...
      function parseTemplate(tpl) {
        var parts = []
        var re = /\{(title|label|duration|usage|error|cache|tps|image)\}/g
        var last = 0
        var m
      ...
      function ChipEditor(props) {
        ...
        function chipEl(key) {
          ...
          el.title = label + '（点击移除）'
          if (key === 'image') {
            // 图片标签：已上传时内嵌小缩略图（data URI），未上传时仅文字占位
            var src = propsRef.current.imageData
            if (src) {
              var th = document.createElement('img')
              th.src = src
              th.alt = ''
              th.style.cssText = 'width:24px;height:15px;object-fit:cover;border-radius:3px;vertical-align:-3px;margin-right:3px'
              el.appendChild(th)
            }
        ...
        // 图片数据变化（上传完成/替换）→ 重建 DOM，让 {image} 标签立即显示新缩略图
        useEffect(function () {
          rebuild(props.template)
      ...
      return function SettingsCard() {
        ...
        var titleInputRef = useRef(null) // 全局推送标题 input（「＋ 插入信息」在光标处插入）
        ...
        /**
         * 本地图片 → data URI：canvas 压缩、JPEG 0.8。
         *  - maxSize：输出最长边（大图 512、图标 128）；
         *  - cropAspect：>0 时按该比例**中心裁切（cover）**后缩放到 maxSize 宽——通知显示区域是固定比例的
         *    （Windows toast hero 大图约 16:9、图标 1:1 方形），不裁切的话系统会用显示区域硬裁，出现
         *    「只显示图的一部分」；裁切后上传的图 = 通知里显示的图（所见即所得）。
         *    原图已小于输出宽时不放大（保持锐利，显示时由系统放大）。
         *  - cropAspect = 0/null：等比缩放不裁切（保留原比例）。
         * 单张约 30~80KB，避免设置文档膨胀。
         */
        ...
        /** 全局通知大图/图标的本地上传：压缩（含裁切）后写入 imageUrl/iconUrl 字段（data URI 与 https URL 并存）。 */
        function pickMediaUrl(key, maxSize, cropAspect) {
          try {
            ...
            input.onchange = function () {
              ...
              compressImageFile(file, function (dataUri) {
                input.remove()
                if (dataUri) {
                  setVal(key, dataUri)
                  log('media uploaded for ' + key + ' (' + Math.round(dataUri.length / 1024) + 'KB)')
                } else {
        ...
        /** 通知媒体输入行：本地上传后显示缩略图预览 + https URL 输入框 + 上传/替换/清除按钮；
         *  值已是 data URI（本地上传）时隐藏长串、显示占位提示（避免长 base64 串占满面板）。 */
        function mediaFieldRow(labelKey, key, maxSize, cropAspect) {
          var val = String(cur[key] ?? '')
          var isData = val.indexOf('data:') === 0
          ...
          var btnStyle = {
            ...
            background: 'none', color: 'var(--dsw-alias-label-secondary, inherit)', flex: 'none',
          }
          return fieldRow(t[labelKey], h('div', { style: { display: 'flex', gap: '6px', alignItems: 'center' } },
            isData
              ? h('img', {
                src: val, alt: '', title: t.uploadedHint,
                style: {
                  width: square ? '32px' : '88px', height: square ? '32px' : '50px',
                  objectFit: 'cover', borderRadius: '6px', flex: 'none',
                  border: '1px solid var(--dsw-alias-border-l2, rgba(127,127,127,0.35))',
            ...
            h('input', {
              value: isData ? '' : val,
              placeholder: isData ? t.uploadedHint : (labelKey === 'imageUrl' ? t.imageHint : t.iconHint),
              onChange: function (e) { setVal(key, e.target.value) },
              style: Object.assign({}, SELECT_STYLE, { flex: 1, fontSize: '13px' }),
            }),
            h('button', {
              type: 'button', title: t.imagePickTitle,
              onClick: function () { pickMediaUrl(key, maxSize, cropAspect) },
              style: btnStyle,
            }, isData ? t.replaceMedia : t.upload),
            isData
              ? h('button', { type: 'button', title: t.clearMedia, onClick: function () { setVal(key, '') }, style: btnStyle }, t.clearMedia)
              : null,
          ), key)
        }
        ...
        function pickImage(field) {
          try {
            ...
            input.onchange = function () {
              var file = input.files && input.files[0]
              // 按原因大图：512px 宽、16:9 中心裁切（与 Windows 通知大图显示比例一致）
              compressImageFile(file, function (dataUri) {
                input.remove()
                if (dataUri) {
                  setVal('img-' + field, dataUri)
                  log('image picked for ' + field + ' (' + Math.round(dataUri.length / 1024) + 'KB)')
                } else {
                  setNote(t.imageFailed)
                  log('image pick failed for ' + field)
                }
              }, 512, 16 / 9)
            }
            input.click()
        ...
        function sendTestNotice(field) {
          try {
            ...
            var title = String(titleTpl).replace(/\{image\}/g, '').replaceAll('{title}', samples.title).trim() || samples.title
            var tpl = templateOf(cur, field)
            var bodyTpl = tpl !== '' ? tpl : defaultHintOf(field, cur.language)
            var body = String(bodyTpl)
              .replace(/\{label\}/g, '')
              .replace(/\{image\}/g, '') // {image} 是开关标签：正文只剥除，图片由 notifyUser 按 imageData 附加
        ...
        if (!ready) {
          ...
          var rows = [
            ...
            // 值支持 https URL 或本地上传（压缩后的 data URI，输入框隐藏长串、显示占位提示）
            ...
            mediaFieldRow('imageUrl', 'imageUrl', 512, 16 / 9),
            mediaFieldRow('iconUrl', 'iconUrl', 128, 1),
            h('div', { key: 'title-row', style: { display: 'flex', flexDirection: 'column', gap: '6px', padding: '12px 0', borderTop: DIVIDER } },
              ...
              // 推送标题的信息插入菜单：与原因栏同款（可插 {title}{duration}{usage}{error}{cache}{tps}，不含图片——标题是文字输入框）
              titleMenuOpen
                ? h('div', {
                  key: 'title-menu',
                  style: {
                  ...
                  return h('button', {
                    ...
                    onClick: function () {
                      ...
                      var next = val.slice(0, pos) + '{' + k + '}' + val.slice(pos)
                      setVal('titleTemplate', next)
                      setTimeout(function () {
                        if (el) { el.focus(); var np = pos + k.length + 2; el.setSelectionRange(np, np) }
                      }, 0)
                    },
                    style: {
