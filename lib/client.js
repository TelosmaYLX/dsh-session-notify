...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *  注意 uiSession 不能进 inject：cordis 的 inject 是硬依赖，服务缺席时整个客户端
     *  fiber 永久 park（apply 不跑 → 通知与设置面板一起消失），而该服务只在部分宿主
     *  版本提供。改用 ctx.get('uiSession') 可选查找（见下），缺席时自动降级到投影/快照信号。 */
    ...
    var APPROVAL_PROJECTION_KEY = 'session-complete-notify-approval' // 审批中投影 key（客户端据此推送审批提醒）
    ...
    var I18N = {
      zh: {
        ...
        presets: { custom: '自定义预设', unnamed: '未命名预设', default: '默认', kaomoji: '颜文字', felyne: '艾露猫', neko: '猫娘', deepseekChan: 'DeepSeek 娘' },
        ...
        discard: '放弃', save: '保存', saving: '保存中…', saved: '已保存 ✓ · 刷新页面后生效', customDeleteConfirm: '确定删除该自定义预设？此操作不可恢复。', presetDeletedKeepApplied: '已应用的配置不受影响；如需恢复默认请点「重置」', presetNameExists: '已存在同名预设，请换一个名称', saveDuringEdit: '已保存，但保存期间有新修改未保存。', noChange: '当前配置与默认一致，无需保存', saveFailed: '保存失败：', refresh: '点击刷新', reset: '重置', resetDone: '已重置为默认值 · 刷新页面后生效',
      ...
      'zh-tw': {
        ...
        presets: { custom: '自訂預設', unnamed: '未命名預設', default: '預設', kaomoji: '顏文字', felyne: '艾露貓', neko: '貓娘', deepseekChan: 'DeepSeek 娘' },
        ...
        discard: '放棄', save: '儲存', saving: '儲存中…', saved: '已儲存 ✓ · 重新整理頁面後生效', customDeleteConfirm: '確定刪除該自訂預設？此操作不可復原。', presetDeletedKeepApplied: '已套用的設定不受影響；如需恢復預設請點「重設」', presetNameExists: '已存在同名預設，請換一個名稱', saveDuringEdit: '已儲存，但儲存期間有新修改未儲存。', noChange: '目前設定與預設一致，無需儲存', saveFailed: '儲存失敗：', refresh: '點擊重新整理', reset: '重置', resetDone: '已重置為預設值 · 重新整理頁面後生效',
      ...
      en: {
        ...
        presets: { custom: 'Custom preset', unnamed: 'Untitled preset', default: 'Default', kaomoji: 'Kaomoji', felyne: 'Felyne (MH cat)', neko: 'Neko girl', deepseekChan: 'DeepSeek-chan' },
        ...
        discard: 'Discard', save: 'Save', saving: 'Saving…', saved: 'Saved ✓ · refresh page to apply', customDeleteConfirm: 'Delete this custom preset? This cannot be undone.', presetDeletedKeepApplied: 'The applied configuration is unaffected; to restore defaults, click Reset', presetNameExists: 'A preset with this name already exists; please use another name', saveDuringEdit: 'Saved, but there are unsaved changes made during saving.', noChange: 'Current config matches the default; nothing to save.', saveFailed: 'Save failed: ', refresh: 'Refresh', reset: 'Reset', resetDone: 'Reset to defaults ✓ · refresh page to apply',
      ...
      ja: {
        ...
        presets: { custom: 'カスタムプリセット', unnamed: '未命名プリセット', default: 'デフォルト', kaomoji: '顔文字', felyne: 'アイルー', neko: 'ネコ娘', deepseekChan: 'DeepSeekちゃん' },
        ...
        discard: '破棄', save: '保存', saving: '保存中…', saved: '保存しました ✓ · ページを再読み込み', customDeleteConfirm: 'このカスタムプリセットを削除しますか？この操作は取り消せません。', presetDeletedKeepApplied: '適用済みの設定には影響しません。デフォルトに戻すには「リセット」をクリック', presetNameExists: '同名のプリセットが既に存在します。別の名前を付けてください', saveDuringEdit: '保存しましたが、保存中に新しい変更があります（未保存）。', noChange: '現在の設定はデフォルトと同じです。保存の必要はありません', saveFailed: '保存失敗：', refresh: '再読み込み', reset: 'リセット', resetDone: 'デフォルトに戻しました ✓ · ページを再読み込み',
      ...
      ko: {
        ...
        presets: { custom: '사용자 지정 프리셋', unnamed: '이름 없는 프리셋', default: '기본', kaomoji: '카오모지', felyne: '아이루', neko: '네코무스메', deepseekChan: 'DeepSeek짱' },
        ...
        discard: '포기', save: '저장', saving: '저장 중…', saved: '저장됨 ✓ · 새로고침 후 적용', customDeleteConfirm: '이 사용자 지정 프리셋을 삭제할까요? 이 작업은 되돌릴 수 없습니다.', presetDeletedKeepApplied: '적용된 구성에는 영향이 없습니다. 기본값으로 되돌리려면 「초기화」를 클릭하세요', presetNameExists: '같은 이름의 프리셋이 이미 있습니다. 다른 이름을 사용하세요', saveDuringEdit: '저장되었지만, 저장 중에 새로 수정된 내용이 있어 아직 저장되지 않았습니다.', noChange: '현재 설정이 기본값과 동일합니다. 저장할 필요가 없습니다', saveFailed: '저장 실패: ', refresh: '새로고침', reset: '초기화', resetDone: '기본값으로 초기화됨 ✓ · 새로고침 후 적용',
    ...
    exports.apply = function apply(ctx) {
      ...
      var pushedApprovals = new Map() // sessionId -> 已推送的审批片段（信号来源标记，避免重复推送）
      var uiPending = null // uiSession.pendingInteractions（可选服务，ctx.get 取到才赋值；缺席保持 null）
      ...
      function onSnapshot() {
        ...
        for (var i = 0; i < entries.length; i++) {
          ...
          checkQuestion(id, s)
          // 审批推送同口径：approval/asked 期间 running 仍为 true，不能走边沿检测。
          checkApproval(id, s)
          if (!primed) continue
      ...
      /**
       * 提问推送：优先读会话投影里的 pending ask_user_question（含 5 语言默认/自定义
       * 标题与正文、问题全文）。同一轮提问只推一次；问题被 tool/result 或 turn/end
       * 清除后重置记录。
       *
       * 兜底：宿主投影是「tool/call → session-complete-notify-question」链路，部分
       * harness 版本（如 0.1.2）下该投影可能不到达客户端，或 ask_user_question 不再
       * 走 tool/call——此时完成通知尚有「会话事件窗口」可兜底，提问通知却无任何兜底，
       * 导致提问从不弹窗。而 harness 原生有独立于本插件投影的「待提问」交互标记
       * `pendingInteraction`（由 question/requested 帧驱动，即提问弹窗本体），任意
       * harness 版本都必发，故以其作第二触发源：投影缺失但存在待提问时也提醒一次，
       * 保证「提问时至少弹一个提示」。
       *
       * 去重：pushedQuestions 记「本会话当前是否已为该轮提问提醒过」，无论来自投影
       * 还是 pendingInteraction 兜底，一律只推一次；问题 resolved 后（两个信号都消失）
       * 清除记录，下个问题可再弹。避免投影先清除、pendingInteraction 仍挂着的瞬间
       * 误触发第二次（通用文案）提醒。
       */
      function checkQuestion(id, s) {
        ...
        } catch (e) { /* 投影缺失 → 走 pendingInteraction 兜底 */ }
        var hasProjection = !!(pv && typeof pv === 'object' && pv.id && pv.text)
        var pending = s.pendingInteraction
        var hasPending = (pending === 'question' || pending === 'plan-review')
        var active = hasProjection || hasPending
        if (!active) {
          if (pushedQuestions.has(id)) pushedQuestions.delete(id)
          return
        }
        if (pushedQuestions.has(id)) return // 已为当前提问提醒过（投影/兜底二选一，不重复弹）
        pushedQuestions.set(id, true)
        if (hasProjection) {
          ...
          log('question push (projection): ' + id + ' ' + pv.id + ' ' + title)
          notifyUser(title, body, pushModeOf(), mediaOf('question'))
          return
        }
        // 兜底：宿主投影缺失，用 harness 原生「待提问」交互标记触发提醒。
        var fname = s.displayTitle || s.title || id
        var ftitle = fname ? (fname + ' · AI 正在向你提问') : 'AI 正在向你提问'
        var fbody = fname ? '会话「' + fname + '」正在等待你的回答，请前往查看。' : 'AI 正在向你提问，请前往查看。'
        log('question push (pendingInteraction fallback): ' + id + ' ' + pending)
        notifyUser(ftitle, fbody, pushModeOf(), mediaOf('question'))
      }

      /**
       * 审批推送（三信号优先级）：
       *  1. uiSession.pendingInteractions（harness 原生挂起交互，approval/asked 即注册、
       *     decided 即移除；仅部分宿主提供该服务，缺席时跳过——含 toolName/reason）；
       *  2. 宿主审批投影（approval/asked 时写入，title/body 宿主已渲染，含 toolName/reason）；
       *  3. 列表快照 pendingInteraction === 'approval'（各宿主通用的折叠字段）。
       * 同一会话的同一审批片段只推一次；审批结束（所有信号消失）后清除记录。
       */
      function approvalSignalOf(id, s) {
        try {
          var it = uiPending && uiPending.getSnapshot && uiPending.getSnapshot().get(id)
          if (it && it.kind === 'approval') return { source: 'uiPending', it: it }
        } catch (e) { /* 老宿主无 uiSession → 走后续信号 */ }
        try {
          var pv = s.projectionValues && s.projectionValues[APPROVAL_PROJECTION_KEY]
          if (pv && typeof pv === 'object' && pv.id) return { source: 'projection', pv: pv }
        } catch (e) { /* 投影缺失 → 走快照字段 */ }
        if (s.pendingInteraction === 'approval') return { source: 'snapshot' }
        return null
      }

      function checkApproval(id, s) {
        if (s.origin === 'subagent' && subagentSkipped()) return // 子代理审批由父会话编排（可取消跳过）
        var sig = approvalSignalOf(id, s)
        if (!sig) {
          if (pushedApprovals.has(id)) pushedApprovals.delete(id) // 审批结束，下个审批可再弹
          return
        }
        if (pushedApprovals.has(id)) return // 已为当前审批片段提醒过，不重复弹
        pushedApprovals.set(id, sig.source)
        var name = s.displayTitle || s.title || id
        var title
        var body
        if (sig.source === 'projection') {
          // 标题/正文：宿主已渲染（title 不含会话名，客户端与提问通道同口径拼 displayTitle）
          var pv = sig.pv
          title = (typeof pv.title === 'string' && pv.title.trim() !== '') ? (name ? name + ' · ' : '') + pv.title.trim() : (name ? name + ' 需要权限审批' : '需要权限审批')
          body = (typeof pv.body === 'string' && pv.body.trim() !== '') ? pv.body.trim() : (name ? '会话「' + name + '」正在等待权限审批。' : '正在等待权限审批。')
        } else {
          // uiPending / 快照兜底：PendingApproval 带 toolName 与可选 reason，客户端本地渲染
          var tool = sig.it && sig.it.toolName ? String(sig.it.toolName) : ''
          var reason = sig.it && typeof sig.it.reason === 'string' ? sig.it.reason.trim() : ''
          title = name ? (name + ' · 需要权限审批') : '需要权限审批'
          body = name ? ('会话「' + name + '」' + (tool ? '请求使用 ' + tool : '正在等待权限审批') + (reason ? '（原因：' + reason + '）' : '') + '，请前往审批。')
                      : ((tool ? '请求使用 ' + tool : '') + '正在等待权限审批，请前往查看。')
        }
        log('approval push (' + sig.source + '): ' + id + ' ' + title)
        notifyUser(title, body, pushModeOf(), mediaOf('approval'))
      }
      ...
      function pushTitle(name, projTitle, suffix) {
        ...
        // P1-5：与宿主 renderTitle 对齐——替换全部信息位，缺数据时替换为空串（剥除），避免 {duration} 等字面量泄漏到通知标题
        var out = String(tpl)
          .replace(/\{image\}/g, '').replace(/\{icon\}/g, '').replace(/\{label\}/g, '')
          .replaceAll('{title}', name)
          .replace(/\{duration\}/g, '').replace(/\{usage\}/g, '').replace(/\{error\}/g, '').replace(/\{cache\}/g, '').replace(/\{tps\}/g, '')
          .trim()
        if (out === '') out = '任务已完成'
        return out + (suffix || '')
      }
      ...
      // 审批可选信号源：uiSession.pendingInteractions（独立于列表快照，approval/asked
      // 即注册、decided 即移除）。用 ctx.get 可选查找而非 ctx.uiSession——动态客户端
      // 上下文只放行 inject 里声明过的服务，直接取未声明服务会被守卫拒绝；ctx.get
      // 是官方给出的可选读取口，服务缺席返回 undefined。订阅后任何挂起交互变化都会
      // 重跑 onSnapshot 全量扫描；缺席时审批退化为投影/快照信号。
      var disposeUiPending = null
      try {
        var uiSession = typeof ctx.get === 'function' ? ctx.get('uiSession') : undefined
        if (uiSession && uiSession.pendingInteractions && typeof uiSession.pendingInteractions.subscribe === 'function') {
          uiPending = uiSession.pendingInteractions
          disposeUiPending = uiPending.subscribe(function () { onSnapshot() })
        }
      } catch (e) { /* 宿主不提供 uiSession：审批退化为快照/投影信号 */ }
      ...
      try {
        if (typeof window !== 'undefined') {
          debugHook = {
            ...
            /** 审批/提问链路探针：列出每个会话的 pendingInteraction 与两个投影的到达情况。 */
            approvalState: function () {
              try {
                ...
                var out = []
                var uiMap = null
                try { uiMap = uiPending && uiPending.getSnapshot ? uiPending.getSnapshot() : null } catch (e) { /* 老宿主 */ }
                Object.keys(byId).forEach(function (k) {
                  var s = byId[k]
                  var apv = null
                  var qpv = null
                  try { apv = s.projectionValues && s.projectionValues[APPROVAL_PROJECTION_KEY] } catch (e) { /* 缺失 */ }
                  try { qpv = s.projectionValues && s.projectionValues[QUESTION_PROJECTION_KEY] } catch (e) { /* 缺失 */ }
                  var uiIt = null
                  try { uiIt = uiMap && uiMap.get ? uiMap.get(k) : null } catch (e) { /* 缺失 */ }
                  out.push({
                    id: String(k).slice(0, 8),
                    title: s.displayTitle || s.title || '',
                    origin: s.origin,
                    running: s.running,
                    pending: s.pendingInteraction || null,
                    uiPending: uiIt && uiIt.kind ? uiIt.kind + (uiIt.toolName ? '/' + uiIt.toolName : '') : null,
                    approvalProj: apv && apv.id ? String(apv.id).slice(0, 8) + '/' + (apv.toolName || '') : null,
                    questionProj: qpv && qpv.id ? String(qpv.id).slice(0, 8) : null,
                  })
                })
      ...
      function cleanup() {
        ...
        if (disposeUiPending !== null) {
          try { disposeUiPending() } catch (e) { /* 订阅已取消则忽略 */ }
          disposeUiPending = null
        }
        if (retryTimer !== null) {
    ...
    function makeSettingsCard(react, ctx) {
      ...
      var PRESETS = [
        ...
        // 载入/切换语言时自动取当前语言的文案（缺语言回退 zh）
        {
          ...
          i18n: {
            zh: {
              titleTemplates: { completed: '任务完成 (＾▽＾)', error: '任务出错 (；д；)', aborted: '任务中止 (；ω；)', blocked: '任务被阻塞 (・_・;)', 'max-tokens': '任务到上限 (￣▽￣)', question: 'AI 提问 (・ω・)ノ' },
              templates: {
                ...
                'max-tokens': '会话「{title}」达到上限了 (￣▽￣)ゞ，建议拆分任务，请点击查看。',
                question: 'AI 向你提问 (・ω・)ノ：{question}',
              },
            },
            'zh-tw': {
              titleTemplates: { completed: '任務完成 (＾▽＾)', error: '任務出錯 (；д；)', aborted: '任務中止 (；ω；)', blocked: '任務被阻塞 (・_・;)', 'max-tokens': '任務到上限 (￣▽￣)', question: 'AI 提問 (・ω・)ノ' },
              templates: {
                ...
                'max-tokens': '會話「{title}」達到上限了 (￣▽￣)ゞ，建議拆分任務，請點擊查看。',
                question: 'AI 向你提問 (・ω・)ノ：{question}',
              },
            },
            en: {
              titleTemplates: { completed: 'Task done (＾▽＾)', error: 'Task failed (；д；)', aborted: 'Task aborted (；ω；)', blocked: 'Task blocked (・_・;)', 'max-tokens': 'Task hit the cap (￣▽￣)', question: 'AI asks (・ω・)ノ' },
              templates: {
                ...
                'max-tokens': 'Session "{title}" hit the cap (￣▽￣)ゞ — consider splitting the task. Click to view.',
                question: 'AI is asking you (・ω・)ノ: {question}',
              },
            },
            ja: {
              titleTemplates: { completed: 'タスク完了 (＾▽＾)', error: 'タスクエラー (；д；)', aborted: 'タスク中止 (；ω；)', blocked: 'タスクブロック (・_・;)', 'max-tokens': '上限到達 (￣▽￣)', question: 'AI からの質問 (・ω・)ノ' },
              templates: {
                ...
                'max-tokens': 'セッション「{title}」が上限に到達 (￣▽￣)ゞ — タスク分割をおすすめ。クリックして表示。',
                question: 'AI からの質問 (・ω・)ノ：{question}',
              },
            },
            ko: {
              titleTemplates: { completed: '작업 완료 (＾▽＾)', error: '작업 오류 (；д；)', aborted: '작업 중단 (；ω；)', blocked: '작업 차단 (・_・;)', 'max-tokens': '한도 도달 (￣▽￣)', question: 'AI 질문 (・ω・)ノ' },
              templates: {
                ...
                'max-tokens': '세션 "{title}" 한도 도달 (￣▽￣)ゞ — 작업 분할 권장. 클릭하여 확인.',
      ...
      function matchesPreset(cur, p) {
        ...
        // 内置预设 language 为 null → 任意语言均可匹配（多语言风格预设）；default 等固定语言预设按原规则
        if (p.language !== null && p.language !== undefined && p.language !== lang) return false
        if (String(cur.titleTemplate ?? '') !== String(p.titleTemplate ?? '')) return false
        // 通知媒体参与匹配：上传了全局大图/图标的配置 ≠ 无图的默认/旧预设（否则保存后重开面板会误匹配默认）
        if (String(cur.imageUrl ?? '') !== String(p.imageUrl ?? '')) return false
        if (String(cur.iconUrl ?? '') !== String(p.iconUrl ?? '')) return false
        var loc = p.i18n ? (p.i18n[lang] || p.i18n.zh || p) : p
        var pt = (loc.templates && typeof loc.templates === 'object') ? loc.templates : {}
        var ptt = (loc.titleTemplates && typeof loc.titleTemplates === 'object') ? loc.titleTemplates : {}
        var pimg = (p.images && typeof p.images === 'object') ? p.images : {}
        var picon = (p.icons && typeof p.icons === 'object') ? p.icons : {}
        for (var j = 0; j < REASON_FIELDS.length; j++) {
          var f = REASON_FIELDS[j]
          if (templateOf(cur, f) !== String(pt[f] ?? '').trim()) return false
          if (titleTplOf(cur, f) !== String(ptt[f] ?? '').trim()) return false
          // 按原因图片/图标数据也参与匹配（P1-4：用 imageOf/iconOf 兼容扁平草稿键与宿主嵌套结构，
          // 否则刷新后（宿主 value 为嵌套 images/icons）扁平键恒空 → 匹配失败 → 下拉显示「自定义」）
          if (imageOf(cur, f) !== String(pimg[f] ?? '')) return false
          if (iconOf(cur, f) !== String(picon[f] ?? '')) return false
        }
        return true
      }
      ...
      function ChipEditor(props) {
        ...
        function removeChipAt(el) {
          ...
          // contentEditable 中标签/文本之间常有空文本节点或 <br>（换行占位）。
          // 跳过它们定位光标，让光标落在真实内容旁（可视位置正确）。
          // 注意：连环删标签的真正根因在 onKeyDown 找胶囊时跳过了非空文本节点，见那里。
          while (next && ((next.nodeType === 3 && next.textContent === '') || (next.nodeType === 1 && next.tagName === 'BR'))) next = next.nextSibling
          while (prev && ((prev.nodeType === 3 && prev.textContent === '') || (prev.nodeType === 1 && prev.tagName === 'BR'))) prev = prev.previousSibling
          el.remove()
          if (editor) {
          ...
          onKeyDown: function (e) {
            ...
            if (e.key === 'Backspace' || e.key === 'Delete') {
              ...
              if (range && range.collapsed) {
                ...
                // 只跳过「空文本节点」找胶囊；一遇到非空文本必须停——
                // 此时光标紧邻的是真实文字，应交给浏览器删字符。
                // 若跳过文字继续找胶囊，连按 Back 会把标签全删光、删完标签才轮到文字
                //（例：【A】123【B】456【C】末尾按 3 下：修复前删掉 C、B、A 得 "123456"；
                //  修复后删掉 C、"6"、"5" 得 【A】123【B】4）。
                while (target && target.nodeType === 3 && target.textContent === '') target = e.key === 'Backspace' ? target.previousSibling : target.nextSibling
      ...
      return function SettingsCard() {
        ...
        // 最新 draft 引用：保存回调等异步路径读取「此刻」的草稿（闭包里的 draft 是触发时快照，P0-3）
        ...
        var tplOpenState = useState(true) // 「内容」折叠区展开状态（默认展开：打开面板即可编辑各原因正文）
        ...
        function loadPreset(entry, custom) {
          ...
          // 多语言风格预设：按当前语言取文案；language 为 null/空 → 保留当前界面语言（不强制切换）
          var lang = LANG_IDS.indexOf(cur.language) >= 0 ? cur.language : 'zh'
          var loc = entry.i18n ? (entry.i18n[lang] || entry.i18n.zh || entry) : entry
          next.language = entry.language || cur.language
          next.titleTemplate = String(entry.titleTemplate ?? '')
          var lt = (loc.templates && typeof loc.templates === 'object') ? loc.templates : {}
          for (var j = 0; j < REASON_FIELDS.length; j++) next['tpl-' + REASON_FIELDS[j]] = typeof lt[REASON_FIELDS[j]] === 'string' ? lt[REASON_FIELDS[j]] : ''
          // 按原因定制标题（旧预设可能没有 → 空）
          var et = (loc.titleTemplates && typeof loc.titleTemplates === 'object') ? loc.titleTemplates : {}
        ...
        function saveAsCustom() {
          ...
          // P2-2：同名预设提示（下拉同名条目无法区分）
          for (var ni = 0; ni < customPresets.length; ni++) {
            if (String(customPresets[ni].name ?? '').trim() === name) { setNote(t.presetNameExists); return }
          }
          var flat = flatOf(cur)
          var images = {}
          var icons = {}
          ...
          var cp = {
            // P2-1：加随机后缀，避免同毫秒两次另存为 id 冲突（与 renameCustom 一致）
        ...
        function deleteCustom() {
          ...
          // P0-1：误点即永久丢失（预设含 5 原因模板 + 图片 data URI）→ 删除前确认
          if (!window.confirm(t.customDeleteConfirm)) return
          var next = customPresets.filter(function (cp) { return cp.id !== shownEntry.id })
          setCustomPresets(next)
          saveCustomPresets(next)
          setActivePreset(null)
          // 删除的是当前使用的预设：表单恢复为**宿主已保存值**（而非默认值）——
          // 避免「表单变默认 → dirty → 用户误点保存 → 宿主配置被覆盖为默认」的不可恢复覆盖（P0-2）；
          // 宿主值含当前语言，天然满足「删除保留语言」（P1-1）
          if (shownEntry.id === (activePreset && activePreset.id)) {
            setDraft(Object.assign({}, value))
            setNote(t.presetDeletedKeepApplied) // 「已应用配置不受影响」提示
          } else {
        ...
        function save() {
          ...
          // P1-3：串行写 scope 字段（失败即停，明确已写数量；不再 Promise.all 静默部分成功）
          var serialDone = 0
          var chain = Promise.resolve()
          for (var si2 = 0; si2 < tasks.length; si2++) {
            ;(function (task) {
              chain = chain.then(function () { return task }).then(function () { serialDone++ })
            })(tasks[si2])
          }
          chain.then(function () {
            // P0-3：保存快照——回调使用触发保存时的值，避免保存期间用户操作（删预设/编辑）导致闭包陈旧竞态
            var snapCustom = customPresets.slice()
            var snapShownCustom = shownCustom
            var snapShownEntry = shownEntry
            var snapFlat = flatOf(cur)
            // 预设自动同步：
            //  - 来源是自定义预设（含未命名预设）→ 更新它；
            //  - 来源是默认/空白 → 【若已有未命名预设则继续编号新建】未命名预设 2/3…
            var lang = LANG_IDS.indexOf(snapFlat.language) >= 0 ? snapFlat.language : 'zh'
            var targetId
            var entry
            var list
            var found = null
            // 通知媒体随预设一并保存（全局 + 按原因）：否则保存后的新/更新预设丢图，
            // 且重开面板时 presetIdOf 会误匹配无图的默认预设
            var media = {
              imageUrl: String(snapFlat.imageUrl ?? ''),
              iconUrl: String(snapFlat.iconUrl ?? ''),
              imagePreviewUrl: String(snapFlat['preview-imageUrl'] ?? ''),
              iconPreviewUrl: String(snapFlat['preview-iconUrl'] ?? ''),
              images: images,
              icons: icons,
              imagePreviews: imagePreviews,
              iconPreviews: iconPreviews,
            }
            if (snapShownCustom && snapShownEntry) {
              targetId = snapShownEntry.id
              entry = Object.assign({}, snapShownEntry, { language: lang, titleTemplate: snapFlat.titleTemplate, titleTemplates: titleTemplates, templates: templates }, media)
              list = snapCustom.map(function (cp) {
                if (cp.id === targetId) { found = Object.assign({}, cp, entry); return found }
                return cp
              })
              if (!found) { list = list.concat([entry]); found = entry }
            } else {
              // P2-6：当前配置与默认完全一致（未做任何修改）→ 保存无意义，跳过重复新建
              if (presetIdOf(cur, customPresets) === 'default') {
                setNote(t.noChange)
                setDraft(null)
                setJustSaved(true)
                log('settings saved (matches default); no new preset created')
                return
              }
              var n = nextUnnamedNumber(snapCustom)
              targetId = n === 1 ? 'unnamed' : 'unnamed-' + n
              entry = Object.assign({ id: targetId, name: null, auto: true, language: lang, titleTemplate: snapFlat.titleTemplate, titleTemplates: titleTemplates, templates: templates }, media)
              list = snapCustom.concat([entry])
              found = entry
            }
            setCustomPresets(list)
            saveCustomPresets(list)
            setActivePreset({ id: targetId, custom: true, src: found })
            // P0-3：保存期间用户是否又改了草稿？改了则保留草稿并提示（不清空用户输入）
            var nowDraft = draftRef.current
            if (nowDraft !== null && !sameFlat(flatOf(nowDraft), snapFlat)) {
              setNote(t.saveDuringEdit)
              setJustSaved(true)
              log('settings saved; preset synced: ' + targetId + ' (edits during save kept)')
            } else {
              setDraft(null)
              setNote(t.saved)
              setJustSaved(true)
              log('settings saved; preset synced: ' + targetId + ' (wrote ' + serialDone + '/' + tasks.length + ')')
            }
          }).catch(function (e) {
            setNote(t.saveFailed + (e && e.message ? e.message : e) + '（已写入 ' + serialDone + '/' + tasks.length + ' 项）')
            log('settings save failed at ' + serialDone + '/' + tasks.length + ': ' + (e && e.message ? e.message : e))
          })
        }
            ...
            h('div', { style: { position: 'relative', width: w, height: hh } },
              ...
              hasMedia
                ? h('button', {
                  ...
                  // P1-2：裁切版与预览版一并清除，避免 preview-* 残留脏 data URI（与 save() 按原因路径一致）
                  onClick: function () { setVal(key, ''); setVal('preview-' + key, '') },
                  style: {
        ...
        function resetAll() {
          ...
          // P1-3：串行写（与 save() 一致，失败即停并报告已写数量）
          var serialDone = 0
          var chain = Promise.resolve()
          for (var si3 = 0; si3 < tasks.length; si3++) {
            ;(function (task) {
              chain = chain.then(function () { return task }).then(function () { serialDone++ })
            })(tasks[si3])
          }
          chain.then(function () {
            ...
            setNote(t.saveFailed + (e && e.message ? e.message : e) + '（已写入 ' + serialDone + '/' + tasks.length + ' 项）')
            log('settings reset failed at ' + serialDone + '/' + tasks.length + ': ' + (e && e.message ? e.message : e))
          })
        }
        ...
        if (!ready) {
          ...
          rows.push(h('div', {
            ...
            h('button', {
              // P2-5：无修改（!dirty）时禁用重置，避免无意义写库
              type: 'button', disabled: !dirty || note === t.saving, onClick: resetAll,
              style: Object.assign({}, GHOST, { opacity: (!dirty || note === t.saving) ? 0.4 : 1 }),
            }, t.reset),
            h('button', {
