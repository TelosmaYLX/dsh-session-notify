  ...
  factory: (require) => {
    ...
    function makeSettingsCard(react, ctx) {
      ...
      return function SettingsCard() {
        ...
        /**
         * 本地图片 → 通知大图 data URI：canvas 等比压缩至最长边 512px、JPEG 0.8
         * （Windows 通知卡片的大图区域有限，512px 足够清晰；单张约 30~80KB，避免设置文档膨胀）。
         */
        ...
        if (!ready) {
          ...
          var rows = [
            ...
            fieldRow(t.imageUrl, h('input', {
              value: String(cur.imageUrl ?? ''),
              placeholder: t.imageHint,
              onChange: function (e) { setVal('imageUrl', e.target.value) },
              style: Object.assign({}, SELECT_STYLE, { fontSize: '13px' }),
            }), 'imageUrl'),
            fieldRow(t.iconUrl, h('input', {
              value: String(cur.iconUrl ?? ''),
              placeholder: t.iconHint,
              onChange: function (e) { setVal('iconUrl', e.target.value) },
              style: Object.assign({}, SELECT_STYLE, { fontSize: '13px' }),
            }), 'iconUrl'),
