  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        titleLabel: '推送标题', titleHint: '默认「任务已完成」；插入 {title} 使用会话标题',
      ...
      'zh-tw': {
        ...
        titleLabel: '推送標題', titleHint: '預設「任務已完成」；插入 {title} 使用會話標題',
      ...
      en: {
        ...
        titleLabel: 'Push title', titleHint: 'Defaults to "Task completed"; insert {title} for the session title',
      ...
      ja: {
        ...
        titleLabel: 'プッシュタイトル', titleHint: 'デフォルトは「タスク完了」；{title} でセッションタイトルを使用',
      ...
      ko: {
        ...
        titleLabel: '푸시 제목', titleHint: '기본값은 「작업 완료」；{title} 로 세션 제목 사용',
    ...
    exports.apply = function apply(ctx) {
      ...
      /** 推送标题：默认「任务已完成」；用户模板可用 {title} 引用会话标题；无通知时附带完成标记。 */
    ...
    function makeSettingsCard(react, ctx) {
      ...
      var PRESETS = [
        { id: 'default', labelKey: 'default', language: 'zh', titleTemplate: '任务已完成', templates: { completed: '', error: '', aborted: '', blocked: '', 'max-tokens': '' } },
      ]

      /** 重置目标：与宿主 DEFAULT_SETTINGS 一致（设置面板可编辑字段子集）。 */
      var DEFAULT_FLAT = {
        language: 'zh',
        titleTemplate: '任务已完成',
        pushMode: 'dual',
      ...
      return function SettingsCard() {
        ...
        /** 一键重置：把设置还原为默认值（与宿主 DEFAULT_SETTINGS 一致）并立即保存。 */
        function resetAll() {
          var def = DEFAULT_FLAT
          setNote(t.saving)
          var tasks = [
            scope.set('language', def.language),
