...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    exports.apply = function apply(ctx) {
      ...
      var pushedApprovals = new Map() // sessionId -> 已推送的审批 key（投影 approvalId 或 'pending'，避免重复推送）
      ...
      /**
       * 审批推送：优先读宿主审批投影（approval/asked 时写入，approval/decided 或
       * turn/end 清空），兜底 harness 原生「待审批」交互标记
       * （pendingInteraction === 'approval'）。同一审批只推一次；审批结束
       * （两个信号都消失）后清除记录，下一个审批可再弹。
       */
      function checkApproval(id, s) {
        if (s.origin === 'subagent' && subagentSkipped()) return // 子代理审批由父会话编排（可取消跳过）
        var pv = null
        try {
          pv = s.projectionValues && s.projectionValues[APPROVAL_PROJECTION_KEY]
        } catch (e) { /* 投影缺失 → 走 pendingInteraction 兜底 */ }
        var hasProjection = !!(pv && typeof pv === 'object' && pv.id)
        var hasPending = s.pendingInteraction === 'approval'
        if (!hasProjection && !hasPending) {
          if (pushedApprovals.has(id)) pushedApprovals.delete(id)
          return
        }
        var key = hasProjection ? pv.id : 'pending'
        if (pushedApprovals.get(id) === key) return // 已为当前审批提醒过（投影/兜底二选一，不重复弹）
        pushedApprovals.set(id, key)
        var name = s.displayTitle || s.title || id
        if (hasProjection) {
          // 标题/正文：宿主已渲染（title 不含会话名，客户端与提问通道同口径拼 displayTitle）
          var title = (typeof pv.title === 'string' && pv.title.trim() !== '') ? (name ? name + ' · ' : '') + pv.title.trim() : (name ? name + ' 需要权限审批' : '需要权限审批')
          var body = (typeof pv.body === 'string' && pv.body.trim() !== '') ? pv.body.trim() : (name ? '会话「' + name + '」正在等待权限审批。' : '正在等待权限审批。')
          log('approval push (projection): ' + id + ' ' + pv.id + ' ' + title)
          notifyUser(title, body, pushModeOf(), mediaOf('approval'))
          return
        }
        // 兜底：宿主投影缺失，用 harness 原生「待审批」交互标记触发提醒。
        var ftitle = name ? (name + ' · 需要权限审批') : '需要权限审批'
        var fbody = name ? '会话「' + name + '」正在等待权限审批，请前往查看。' : '正在等待权限审批，请前往查看。'
        log('approval push (pendingInteraction fallback): ' + id)
        notifyUser(ftitle, fbody, pushModeOf(), mediaOf('approval'))
      }
