  ...
  factory: (require) => {
    ...
    function makeSettingsCard(react, ctx) {
      ...
      var PRESETS = [
        ...
        {
          id: 'kaomoji', labelKey: 'kaomoji', language: 'zh',
          titleTemplates: { completed: '任务完成 (＾▽＾)', error: '任务出错 (；д；)', aborted: '任务中止 (；ω；)', blocked: '任务被阻塞 (・_・;)', 'max-tokens': '任务到上限 (￣▽￣)' },
          templates: {
            completed: '会话「{title}」已完成 (๑•̀ㅂ•́)و✧（用时 {duration}，消耗 {usage}），请点击查看～',
            error: '会话「{title}」出错啦 (｡•́︿•̀｡)（用时 {duration}，消耗 {usage}），请点击查看。',
            aborted: '会话「{title}」被中止了 (；ω；)（用时 {duration}，消耗 {usage}），请点击查看。',
            blocked: '会话「{title}」被阻塞了 (・_・;)（用时 {duration}，消耗 {usage}），请点击查看。',
            'max-tokens': '会话「{title}」达到上限了 (￣▽￣)ゞ（用时 {duration}，消耗 {usage}），建议拆分任务，请点击查看。',
          },
        },
        {
          id: 'felyne', labelKey: 'felyne', language: 'zh',
          titleTemplates: { completed: '喵～任务完成', error: '喵…任务出错', aborted: '喵？任务中止', blocked: '喵…任务被挡住', 'max-tokens': '喵喵 任务到上限' },
          templates: {
            completed: '喵～会话「{title}」完成啦！用时 {duration}、消耗 {usage}，快去看看吧喵～',
            error: '喵……会话「{title}」出错了喵（用时 {duration}，消耗 {usage}），猎人去看看吧。',
            aborted: '喵？！会话「{title}」中止了喵（用时 {duration}，消耗 {usage}）。',
            blocked: '喵……会话「{title}」被挡住了喵（用时 {duration}，消耗 {usage}）。',
            'max-tokens': '喵喵，会话「{title}」到上限啦喵！用时 {duration}、消耗 {usage}，拆开任务再试喵～',
          },
        },
        {
          id: 'neko', labelKey: 'neko', language: 'zh',
          titleTemplates: { completed: '主人，任务完成喵～', error: '主人…任务出错喵', aborted: '喵！任务中止', blocked: '主人，任务被挡住喵', 'max-tokens': '主人，任务到上限喵' },
          templates: {
            completed: '主人～会话「{title}」已经完成啦喵～（用时 {duration}，消耗 {usage}），快来看看喵！',
            error: '主人……会话「{title}」出错了喵呜（用时 {duration}，消耗 {usage}），人家好担心呢……',
            aborted: '喵！会话「{title}」被中止了喵（用时 {duration}，消耗 {usage}）。',
            blocked: '主人，会话「{title}」被挡住了喵呜（用时 {duration}，消耗 {usage}）。',
            'max-tokens': '主人～会话「{title}」到上限了喵……用时 {duration}、消耗 {usage}，要不要拆开任务喵？',
          },
        },
        {
          id: 'deepseek-chan', labelKey: 'deepseekChan', language: 'zh',
          titleTemplates: { completed: '深酱：任务完成～', error: '深酱：任务出错…', aborted: '深酱：任务中止', blocked: '深酱：任务被阻塞', 'max-tokens': '深酱：任务到上限' },
          templates: {
            completed: '深酱报告！会话「{title}」已完成啦～（用时 {duration}，消耗 {usage}），请点击查看～',
            error: '深酱道歉……会话「{title}」出错了呢（用时 {duration}，消耗 {usage}），非常抱歉！',
            aborted: '深酱遗憾……会话「{title}」被中止了（用时 {duration}，消耗 {usage}）。',
            blocked: '深酱呜咽……会话「{title}」被阻塞了（用时 {duration}，消耗 {usage}）。',
            'max-tokens': '深酱提示！会话「{title}」达到输出上限了～用时 {duration}、消耗 {usage}，建议拆分任务哦！',
          },
        },
      ]
