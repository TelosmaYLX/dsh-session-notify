...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *  UI 会话服务（pendingInteractions——权限审批/提问挂起的权威来源，独立于列表快照）。 */
    exports.inject = ['sessions', 'slots', 'settingsScope', 'uiSession']
    ...
    exports.apply = function apply(ctx) {
      ...
      var uiPending = null // ctx.uiSession.pendingInteractions（apply 内赋值；老宿主缺服务时保持 null）
       ...
       *     decided 即移除，独立于列表快照——0.1.2 上最可靠，含 toolName/reason）；
       *  2. 宿主审批投影（approval/asked 时写入，title/body 宿主已渲染）；
       *  3. 列表快照 pendingInteraction === 'approval'（新 harness 折叠字段）。
      ...
      // 审批权威信号源：uiSession.pendingInteractions（独立于列表快照，approval/asked
      // 即注册、decided 即移除）。订阅后任何挂起交互变化都会重跑 onSnapshot 全量扫描。
      var disposeUiPending = null
      try {
        if (ctx.uiSession && ctx.uiSession.pendingInteractions && typeof ctx.uiSession.pendingInteractions.subscribe === 'function') {
          uiPending = ctx.uiSession.pendingInteractions
          disposeUiPending = uiPending.subscribe(function () { onSnapshot() })
        }
      } catch (e) { /* 老宿主无 uiSession 服务：审批退化为快照/投影信号 */ }
