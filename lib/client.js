    ...
    function makeSettingsCard(react, ctx) {
      ...
      function matchesPreset(cur, p) {
        ...
        for (var j = 0; j < REASON_FIELDS.length; j++) {
          ...
          // 按原因图片/图标数据也参与匹配（与全局 imageUrl/iconUrl 同理，防止保存后误匹配无图的默认预设）
          if (String(cur['img-' + f] ?? '') !== String(pimg[f] ?? '')) return false
          if (String(cur['icon-' + f] ?? '') !== String(picon[f] ?? '')) return false
        }
        return true
      }
        ...
        function save() {
          ...
          Promise.all(tasks).then(function () {
            ...
            var media = {
              ...
              iconUrl: String(flat.iconUrl ?? ''),
              imagePreviewUrl: String(flat['preview-imageUrl'] ?? ''),
              iconPreviewUrl: String(flat['preview-iconUrl'] ?? ''),
              images: images,
              icons: icons,
              imagePreviews: imagePreviews,
              iconPreviews: iconPreviews,
            ...
            if (shownCustom && shownEntry) {
              ...
              entry = Object.assign({}, shownEntry, { language: lang, titleTemplate: flat.titleTemplate, titleTemplates: titleTemplates, templates: templates }, media)
              ...
              entry = Object.assign({ id: targetId, name: null, auto: true, language: lang, titleTemplate: flat.titleTemplate, titleTemplates: titleTemplates, templates: templates }, media)
