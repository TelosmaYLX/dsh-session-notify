...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    exports.apply = function apply(ctx) {
      ...
      function pushCompletion(id, s) {
        ...
        function attempt() {
          ...
          if (proj || Date.now() - start > 6000) {
            retryTimer = null
            notifyUser(pushTitle(name, proj ? proj.title : '', proj ? '' : ' 已完成'), (proj && proj.text) || (cwdLine ? cwdLine + ' · ' : '') + '详情见会话内系统消息', pushModeOf(), mediaOf())
            return
          }
      ...
      /** 通知媒体：设置快照里的自定义大图/图标 URL（https），空串 = 不使用。 */
      function mediaOf() {
        try {
          if (notifyScope) {
            var st = notifyScope.getSnapshot()
            if (st && st.value) return { imageUrl: String(st.value.imageUrl || ''), iconUrl: String(st.value.iconUrl || '') }
          }
        } catch (e) { /* 默认 */ }
      ...
      try {
        if (mode !== 'toast' && typeof Notification !== 'undefined') {
          if (Notification.permission === 'granted') {
            ...
            if (media && media.imageUrl) opts.image = media.imageUrl
      ...
      return function SettingsCard() {
        ...
        function sendTestNotice(field) {
          try {
            ...
            notifyUser(title, body, ['dual', 'system', 'toast'].indexOf(cur.pushMode) >= 0 ? cur.pushMode : 'dual', { imageUrl: String(cur.imageUrl || ''), iconUrl: String(cur.iconUrl || '') })
