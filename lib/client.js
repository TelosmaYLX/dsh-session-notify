  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        titlesGroup: '标题', titleLabel: '推送标题', titleHint: '所有原因共用；留空时按原因用默认标题', titleTplLabel: '按原因定制标题（可选）', tplLabel: '正文模板 × 5', tplCustom: '条已自定义', ttPlaceholders: { completed: '任务已完成', error: '任务出错', aborted: '任务已中止', blocked: '任务被阻塞', 'max-tokens': '任务达到输出上限' },
      ...
      'zh-tw': {
        ...
        titlesGroup: '標題', titleLabel: '推送標題', titleHint: '所有原因共用；留空時按原因用預設標題', titleTplLabel: '按原因自訂標題（選填）', tplLabel: '正文模板 × 5', tplCustom: '條已自訂', ttPlaceholders: { completed: '任務已完成', error: '任務出錯', aborted: '任務已中止', blocked: '任務被阻塞', 'max-tokens': '任務達到輸出上限' },
      ...
      en: {
        ...
        titlesGroup: 'Title', titleLabel: 'Push title', titleHint: 'Shared by all reasons; leave empty for per-reason defaults', titleTplLabel: 'Per-reason titles (optional)', tplLabel: 'Body templates × 5', tplCustom: 'customized', ttPlaceholders: { completed: 'Task completed', error: 'Task failed', aborted: 'Task aborted', blocked: 'Task blocked', 'max-tokens': 'Task hit the output cap' },
      ...
      ja: {
        ...
        titlesGroup: 'タイトル', titleLabel: 'プッシュタイトル', titleHint: '全理由共通；空欄なら理由ごとのデフォルトを使用', titleTplLabel: '理由ごとのタイトル（任意）', tplLabel: '本文テンプレート × 5', tplCustom: '件カスタマイズ済み', ttPlaceholders: { completed: 'タスク完了', error: 'タスクエラー', aborted: 'タスク中止', blocked: 'タスクがブロックされました', 'max-tokens': '出力上限に到達' },
      ...
      ko: {
        ...
        titlesGroup: '제목', titleLabel: '푸시 제목', titleHint: '모든 이유 공통；비우면 이유별 기본값 사용', titleTplLabel: '이유별 제목 (선택)', tplLabel: '본문 템플릿 × 5', tplCustom: '개 사용자 지정됨', ttPlaceholders: { completed: '작업 완료', error: '작업 오류', aborted: '작업 중단됨', blocked: '작업 차단됨', 'max-tokens': '출력 한도 도달' },
        ...
        if (!ready) {
          ...
          var rows = [
            ...
            h('div', { key: 'title-fold', style: { display: 'flex', flexDirection: 'column', gap: '6px', padding: '12px 0', borderTop: DIVIDER } },
              h('button', {
                ...
                h('span', { style: { fontSize: '13px', color: 'var(--dsw-alias-label-secondary, inherit)' } }, t.titlesGroup),
                h('span', { style: { fontSize: '12px', color: SUBTLE } }, titleFoldOpen ? '▾' : '▸'),
              ),
            ),
            titleFoldOpen
              ? h('div', { key: 'title-fold-body', style: { display: 'flex', flexDirection: 'column', gap: '12px', padding: '12px 0', borderTop: DIVIDER } },
                h('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px' } },
                  ...
                  h('input', {
                    ...
                    style: Object.assign({}, SELECT_STYLE, { width: 'min(100%, 380px)', fontSize: '13px' }),
                  }),
                ),
          ...
          rows.push(h('div', {
            ...
            h('button', {
              ...
              h('span', { style: { fontSize: '12px', color: SUBTLE } }, tplOpen ? '▾' : '▸'),
            ),
          ))
