  ...
  factory: (require) => {
    ...
    function makeSettingsCard(react, ctx) {
      ...
      return function SettingsCard() {
        ...
        /**
         * 本地图片 → data URI：canvas 等比压缩至最长边 maxSize（默认 512px）、JPEG 0.8。
         * 大图 512px（Windows 通知卡片大图区域有限）；图标 128px（通知上小图，48px 足够显示）。
         * 单张约 30~80KB，避免设置文档膨胀。
         */
        function compressImageFile(file, cb, maxSize) {
          try {
        ...
        /** 全局通知大图/图标的本地上传：压缩后写入 imageUrl/iconUrl 字段（data URI 与 https URL 并存）。 */
        function pickMediaUrl(key, maxSize) {
          try {
        ...
        function mediaFieldRow(labelKey, key, maxSize) {
          ...
          return fieldRow(t[labelKey], h('div', { style: { display: 'flex', gap: '6px', alignItems: 'center' } },
            ...
            h('button', {
              ...
              onClick: function () { pickMediaUrl(key, maxSize) },
              style: btnStyle,
        ...
        if (!ready) {
          ...
          var rows = [
            ...
            mediaFieldRow('imageUrl', 'imageUrl', 512),
            mediaFieldRow('iconUrl', 'iconUrl', 128),
