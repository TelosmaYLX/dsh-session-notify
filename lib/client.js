  ...
  factory: (require) => {
    ...
    function makeSettingsCard(react, ctx) {
      ...
      return function SettingsCard() {
        ...
        var ttInputRefs = useRef({}) // 按原因定制标题各行的 input 元素（插入信息到光标处）
          ...
          var rows = [
            ...
            titleFoldOpen
              ? h('div', { key: 'title-fold-body', style: { display: 'flex', flexDirection: 'column', gap: '12px', padding: '12px 0', borderTop: DIVIDER } },
                ...
                // 每行带「+」插入按钮（与推送标题同款：只插信息标签，不含图片/图标），插入到光标处
                h('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px' } },
                  ...
                  REASON_FIELDS.map(function (field) {
                    return h('div', { key: 'tt-' + field, style: { display: 'flex', flexDirection: 'column', gap: '4px' } },
                      h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
                        ...
                        h('input', {
                          ref: function (el) { if (el) ttInputRefs.current[field] = el },
