  ...
  try {
    ...
    if (typeof ctx.inject === 'function') {
      ctx.inject(['sessionProjections'], (scoped) => {
        ...
        try {
          // 注册契约（dsh-session-projection）：状态 schema 键名为 stateSchema，
          // 客户端可见性由 wire 块（viewSchema + view）决定——缺 wire 的单元是
          // host-only，snapshot()/onChanged 会跳过它，值永远到不了客户端。
          ...
          rootRegistry.register({
            ...
            wire: { viewSchema: noticeSchema, view: (state) => state },
        ...
        try {
          ...
          rootRegistry.register({
            ...
            wire: { viewSchema: questionSchema, view: (state) => state },
