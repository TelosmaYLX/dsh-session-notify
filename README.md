...
| 通知图片 | 大图两种来源：**按原因上传**——在模板中通过「＋ 插入信息 → 图片」插入 `{image}` 标签并选择本地图片（编辑器内显示为带缩略图的标签，自动压缩至 512px，随各原因独立保存）；**全局 URL**——填 https 图片链接兜底（原因未上传时使用）。**通知图标**（`icon`）填 https 链接，留空用站点默认图标。仅系统通知通道生效（页内 toast 为文字卡片），「发送」测试按钮同样生效 |
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | 新增**自定义通知图片**：模板「＋ 插入信息」新增**图片**标签——插入 `{image}` 并选择本地图片（自动压缩至 512px，编辑器内显示为带缩略图的小标签），按原因独立上传、保存于设置文档（正文渲染时剥除，不进会话日志）；全局「通知大图 URL」与「通知图标 URL」作为设置面板输入框兜底；各原因「发送」测试按钮同样生效；仅系统通知通道生效（页内 toast 为文字卡片） |
