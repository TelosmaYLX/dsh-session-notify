...
| 通知图片 | 两个 URL 输入框：**通知大图**（`image`，显示在系统通知卡片正文中/下方）与**通知图标**（`icon`，通知上的小图标）；填 https 图片链接，留空分别 = 不显示大图 / 用站点默认图标；仅系统通知通道生效（页内 toast 为文字卡片），「发送」测试按钮同样生效 |
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | 新增**自定义通知图片**：设置面板「推送方式」下方新增「通知大图」与「通知图标」两个 URL 输入框——大图（Notification `image`）显示在系统通知卡片正文中/下方，图标（Notification `icon`）替换通知小图标（留空用站点默认）；各原因「发送」测试按钮同样生效；仅系统通知通道生效（页内 toast 为文字卡片） |
