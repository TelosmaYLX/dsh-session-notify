...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| **0.1.17** | 2026-08-30 | **提问即时提醒（可定制版）**：AI 调用 `ask_user_question` 提问时立即弹窗提醒（独立投影 `session-complete-notify-question`，宿主渲染标题+正文，回答后自动失效）；提问文案进入设置面板「标题 / 内容」折叠区（第 6 行「提问」），支持 `{question}` 占位符（注入 AI 实际提问）与 `{image}` / `{icon}` 媒体开关；4 套风格预设（颜文字 / 艾露猫 / 猫娘 / DeepSeek 娘）补齐 5 语言提问文案；老宿主无提问投影时客户端自行拼接兜底 |
| **0.1.16** | 2026-08-30 | **交互修复**：连按 Backspace 不再「先把标签删光才轮到文字」——根因是光标停在文字末尾时，删除逻辑会跳过整段文字误删前方的标签（例：【标签A】123【标签B】456【标签C】末尾按 3 下会变成 123456）；现在只有光标与标签之间没有文字时才删标签，否则正常删字符（同样 3 下得到 【标签A】123【标签B】4；Delete 键同步修复） |
| **0.1.15** | 2026-08-30 | **交互优化**：「内容」折叠区**默认展开**（打开面板即可编辑各原因正文）；删除标签后光标**跳过空文本/换行占位节点**，直接落到真实内容处——不再「连环删除标签、删完标签才能删到文字」，可连贯 Back 连续删除 |
| **0.1.14** | 2026-08-30 | **代码审查修复**：删除预设前弹确认（防误删）；删除当前使用的预设后表单恢复为**宿主已保存配置**（不再重置为默认，避免误点保存覆盖已应用配置，且语言保留）；保存期间的新编辑不再被静默丢弃（提示保留）；保存/重置改串行写入（失败报告已写入项数）；媒体「×」同时清除预览版（无脏 data URI）；按原因图片参与预设匹配（刷新后正确识别带图预设）；客户端降级标题不再泄漏 `{duration}` 等占位符；另存为 id 加随机后缀防冲突；同名预设提示；重置按钮仅在有修改时可用；调试日志超 512KB 自动截断 |
