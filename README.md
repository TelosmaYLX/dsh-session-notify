...
| 通知图片 | 大图两种来源：**按原因上传**——在模板中通过「＋ 插入信息 → 图片」插入 `{image}` 标签并选择本地图片（编辑器内显示为带缩略图的标签，**自动压缩至 512px 宽、按通知显示比例 16:9 居中裁切**，随各原因独立保存）；**全局大图/图标**——支持本地上传（自动压缩：**大图 512×288（16:9 居中裁切）、图标 128×128（1:1 方形居中裁切）**，输入框显示「已上传」占位 + 替换/清除按钮）或填 https URL 兜底（原因未上传时用）。图标留空用站点默认图标，也可在模板中插入 `{icon}` 标签**按原因指定图标**（优先于全局）。仅系统通知通道生效（页内 toast 为文字卡片），「发送」测试按钮同样生效 |
