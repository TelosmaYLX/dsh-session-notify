...
| 内容 | 折叠区（**默认收起**，点击展开）；展开后每条结束原因（完成、出错、中止、阻塞、输出上限）**一行式布局**（原因标签 + Chip 编辑器 + 「+」插入按钮——菜单展开时变「−」+ **纸飞机发送按钮**，按钮为矩形、垂直居中）：文字 + 内联信息胶囊，光标处插入；`{image}`/`{icon}` 标签**点击缩略图可预览大图、点 × 才删除**（防误删），其他标签点击移除 |
