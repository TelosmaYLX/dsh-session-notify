...
| 通知图片 | 大图两种来源：**按原因上传**——在模板中通过「＋ 插入信息 → 图片」插入 `{image}` 标签并选择本地图片（编辑器内显示为带缩略图的标签，自动压缩至 512px，随各原因独立保存）；**全局大图/图标**——支持本地上传（自动压缩：大图 512px、图标 128px，输入框显示「已上传」占位 + 替换/清除按钮）或填 https URL 兜底（原因未上传时用）。图标留空用站点默认图标。仅系统通知通道生效（页内 toast 为文字卡片），「发送」测试按钮同样生效 |
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| **0.1.11** | 2026-08-29 | 新增**自定义通知图片**：① 模板「＋ 插入信息」新增**图片**标签——插入 `{image}` 并选择本地图片（自动压缩至 512px，编辑器内显示为带缩略图的小标签），按原因独立上传、保存于设置文档（正文渲染时剥除，不进会话日志）；② **通知大图/通知图标支持本地上传**（自动压缩：大图 512px、图标 128px，输入框显示「已上传」占位 + 替换/清除按钮，也可填 https URL）；③ **推送标题的「＋ 插入信息」**可插入任意信息位（会话标题/用时/消耗/缓存命中/速度），不再只有会话标题；各原因「发送」测试按钮同样生效；仅系统通知通道生效（页内 toast 为文字卡片） |
